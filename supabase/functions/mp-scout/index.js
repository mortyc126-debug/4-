...
// 2. Глубина донесения (scoutSnapshot, index.html:5237-5282) — Фаза 6,
//    продолжение: подключена по-настоящему в mp-tick (applyScoutArrive),
//    se=p.tech.mil_scout2 (0-5, СЫРОЙ уровень, не bonuses(p).scoutBonus —
//    то поле к глубине не относится). Честно НЕ перенесено: wallHp/wallMax
//    (se>=2, прочность стены как отдельный регенерирующий пул HP —
//    сама эта механика ещё не на сервере, есть только уровень стены);
//    genLv/genTal/gearPower/gear (se>=2/5, генералы/снаряжение не
//    перенесены — честно null/0/[], см. заголовок applyScoutArrive).
