...
const BUILD_MULTI = new Set(["hospital"]);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const p = row.state;
    // Самоисцеление старых записей: до этого шага mp-join заводил p.b.hospital
    // скаляром (0), а не участком [0,0,0,0] (BUILDINGS.hospital.plots=4 в
    // index.html) — на новых join уже исправлено, здесь достраиваем задним
    // числом тех, кто успел зайти раньше.
    if (BUILD_MULTI.has("hospital") && !Array.isArray(p.b.hospital)) p.b.hospital = [p.b.hospital || 0, 0, 0, 0];
