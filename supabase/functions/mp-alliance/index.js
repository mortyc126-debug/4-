...
// Срок стройки при ОДНОМ участнике.
//
// Двенадцатью часами возврата варваров (REGFORT_RESPAWN_SEC в mp-tick) эти
// сроки НЕ ограничены: заложенная стройка таймер варваров ЗАМОРАЖИВАЕТ —
// прямое указание автора. Двенадцать часов — это срок на то, чтобы к
// разорённому месту вообще приступили, а не на то, чтобы успеть достроить.
// Автор про очевидное следствие («союз может так держать место сколько
// угодно») сказал прямо: «при насыщенной игре навряд ли такое будет».
//
// Поэтому сроки взяты по весу самой крепости, а не под чужой таймер: великая
// твердыня — сутки работ у одиночки и около десяти часов у полного союза.
const REGFORT_BUILD_SEC = [0, 6 * 3600, 12 * 3600, 24 * 3600];
