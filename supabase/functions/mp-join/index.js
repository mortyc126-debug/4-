...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!world) {
      ...
      // Фаза 50 — шестнадцать крепостей варваров, по одной на область.
      // Той же одной строкой, что и цепочка respawn'а выше, и по той же
      // причине: заводится ровно раз, при создании мира. Существующий мир
      // сюда не попадёт — ему крепости ставит миграция 0013 отдельным
      // проходом по всем уже заведённым мирам.
      // Координаты и ступени сложности живут в SQL-функции seed_region_forts
      // (та же миграция), а не копией здесь: шестнадцать точек, посчитанных
      // по рельефу (tools/gen_region_forts.mjs), разъехались бы между двумя
      // копиями молча — а место крепости менять нельзя, к ней уже ходят.
      try {
        await admin.rpc("seed_region_forts", { p_world: world.id });
      } catch (_) { /* не критично — крепости ставятся повторным вызовом или миграцией */ }
    }
