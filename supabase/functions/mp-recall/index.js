...
//   Отозванный УЖЕ НА ТОЧКЕ (state:"gather") забирает ТУ ДОЛЮ, что успела
//   набежать, а остаток возвращает точке — дословно как mp-redirect при
//   перетаскивании с недокопанной жилы. Раньше здесь засчитывался весь
//   резерв целиком, и это ломало точку: резерв списан при отправке, назад
//   не возвращался, у точки оставался ноль, а как только марш переставал
//   быть сборщиком, sweepEmptyNodes сносил её насовсем (репорт автора:
//   "отвожу войска с жилы — жила обнуляется и повторно направить туда
//   войска не получается"). Заодно это была дыра: отозвался через секунду —
//   унёс всё, что резервировал на час.
//
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (m.mode === "gather" && m.state === "gather") {
      ...
      // дорога).
      //
      // РАНЬШЕ отзыв с жилы засчитывал ВЕСЬ резерв как собранный, сколько бы
      // отряд там ни простоял. Автор поймал последствия: "когда я отвожу
      // войска с жилы (он ещё не всё собрал), жила обнуляется и повторно
      // направить туда войска не получается". Так и было, и по двум причинам
      // сразу. Резерв (data.take) списывается с точки ещё НА ОТПРАВКЕ (Фаза 8,
      // кусочек 1), поэтому у точки уже стоял ноль; отзыв ничего ей не
      // возвращал, а как только марш переставал быть сборщиком (state:"back"),
      // он выпадал из busy в sweepEmptyNodes — и уборщик сносил пустую точку
      // насовсем. Плюс это была ещё и дыра: отозвался через секунду — унёс
      // всё, что резервировал на час.
      //
      // Теперь берём ровно ту долю, что успела набежать, а остаток
      // возвращаем точке. Формула та же, что клиент УЖЕ показывает в картуше
      // строкой "Собрано X / Y" (см. renderCartoucheMp) — сервер просто
      // перестал расходиться со своим же интерфейсом.
      // Расчёт ДОСЛОВНО тот же, что при перетаскивании отряда с недокопанной
      // точки (mp-redirect, ветка state==="gather"): два пути снятия со сбора
      // обязаны считать одинаково, иначе игрок получал бы разное в
      // зависимости от того, отозвал он отряд кнопкой или увёл пальцем.
      curX = m.tx; curY = m.ty;
      ...
      if (back > 0 && cx != null && cy != null) {
        ...
        // Точки может уже не быть (истощил кто-то другой, уборщик снёс) —
        // тогда возвращать некуда, тот же честный исход, что и у ветки
        // отзыва с дороги ниже.
        if (cell) {
          await admin.from("map_cells")
            .update({ data: { ...(cell.data || {}), amount: ((cell.data && cell.data.amount) || 0) + back } })
