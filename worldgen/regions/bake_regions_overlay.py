...
# заметная.
# 80 (~31%) подобрано на офлайн-рендере ЭТОГО ЖЕ шейдера через настоящий
# WebGPU (tools/render_terrain.py): на 40 territория читается только там, где
# рядом соседний регион другого цвета, а в середине области почти незаметна;
# на 110 и выше заливка уже перекрашивает землю, съедая текстуру и рельеф.
FILL_ALPHA = 80
BORDER_ALPHA = 232
