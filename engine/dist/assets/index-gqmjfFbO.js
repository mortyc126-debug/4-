...
`)}async function Mn(){let e=[];function t(t){jn([...e,t]);try{window.parent&&window.parent!==window&&typeof window.parent.forceCityView==`function`&&window.parent.forceCityView()}catch{}}if(!(`gpu`in navigator)){t(`WebGPU: navigator.gpu отсутствует.`);return}await Lt(),e.push(`рельеф: настоящие данные высот загружены`);let n={x:42,y:22},r=[.6,.52,.4],i=.0016,a=xn(),o=a??n,s=Cn(o.x,o.y,448),c=s!==null;window.parent!==window&&(On.style.display=`none`);let l=s??[{key:`demo-0`,x:43,y:14,gx:43,gy:14,kind:0,model:`/models/castles/human-1.glb`,scale:10,nm:`Замок`,lv:`демо`},{key:`demo-1`,x:50,y:20,gx:50,gy:20,kind:1,model:`/models/camps/barbarians.glb`,scale:5,nm:`Лагерь`,lv:`демо`},{key:`demo-2`,x:55,y:12,gx:55,gy:12,kind:2,model:`/models/resources/farm.glb`,scale:5,nm:`Пашня`,lv:`демо`},{key:`demo-3`,x:30,y:30,gx:30,gy:30,kind:2,model:`/models/resources/quarry.glb`,scale:5,nm:`Каменоломня`,lv:`демо`}];e.push(c?`данные: настоящая партия, сущностей — ${l.length}`:`данные: демо (window.parent.W недоступен)`);let d=u(),f={x:[],y:[]},p={value:[]},m=new Map,h=new Map,g=new Map,_=new Map,v=new Map,y=new Map,b=new Map;function x(e){let t=je(d);return ke(d,t,f),ke(d,t,p),f.x[t]=e.x,f.y[t]=e.y,p.value[t]=e.kind,m.set(t,e.model),h.set(t,e.scale),g.set(t,e.nm),_.set(t,e.lv),v.set(t,!!e.own),y.set(t,{x:e.gx,y:e.gy}),b.set(e.key,t),Gt(e.x,e.y,e.scale*1.4),t}for(let e of l)x(e);let S=Array.from(Ce(d,[f,p]));e.push(`bitECS: сущностей — ${S.length}`);let ee=await navigator.gpu.requestAdapter();if(!ee){t(`WebGPU: адаптер не найден.`);return}let C=await ee.requestDevice();function w(e){let t=document.getElementById(`gpu-error-banner`);t||(t=document.createElement(`div`),t.id=`gpu-error-banner`,t.style.cssText=`position:fixed;left:0;right:0;bottom:0;z-index:99999;background:#4a0f0f;color:#fff;font:11px/1.4 monospace;padding:6px 8px;max-height:40vh;overflow:auto;white-space:pre-wrap;`,document.body.appendChild(t)),t.textContent+=(t.textContent?`
---
`:``)+e}C.addEventListener(`uncapturederror`,e=>{let t=e.error.message;console.error(`WebGPU error:`,t),w(t)});let te=`fb-gpu-reload-at`,ne=Number(sessionStorage.getItem(te)||0),re=ne&&Date.now()-ne<6e4;C.lost.then(e=>{if(console.error(`WebGPU device lost:`,e.reason,e.message),e.reason!==`destroyed`){if(re){w(`WebGPU-устройство теряется повторно (${e.reason}) — похоже, объёмная карта нестабильна на этом устройстве/браузере. Карта города ниже работает независимо от WebGPU.`);try{window.parent&&window.parent!==window&&typeof window.parent.forceCityView==`function`&&window.parent.forceCityView()}catch{}return}w(`WebGPU-устройство потеряно (${e.reason}): ${e.message}\nПерезагрузка через 2с...`),sessionStorage.setItem(te,String(Date.now())),setTimeout(()=>location.reload(),2e3)}});let T=document.getElementById(`gpu`),ie=T.getContext(`webgpu`);if(!ie){t(`WebGPU: getContext('webgpu') вернул null.`);return}let E=navigator.gpu.getPreferredCanvasFormat();function ae(){let e=T.clientWidth,t=T.clientHeight;if(e<=0||t<=0)return;let n=Math.min(2,window.devicePixelRatio||1),r=Math.max(1,Math.floor(e*n)),i=Math.max(1,Math.floor(t*n));T.width!==r&&(T.width=r),T.height!==i&&(T.height=i)}let oe=()=>T.offsetParent===null&&T.clientWidth===0;ae(),new ResizeObserver(ae).observe(T),ie.configure({device:C,format:E,alphaMode:`opaque`}),e.push(`WebGPU: устройство получено, формат — ${E}`);let D=await Et(C,ie,E);function se(e,t){return e+`,`+t}function ce(e,t){try{let n=window.parent;n&&n!==window&&typeof n.ensureWorldChunk==`function`&&n.ensureWorldChunk(e,t)}catch{}}let le=new Map;function O(e,t){return[e[0]*t,e[1]*t,e[2]*t]}function ue(e,t,n){if(Kt(e,t))return!0;for(let r=0;r<8;r++){let i=r/8*Math.PI*2;if(Kt(e+Math.cos(i)*n,t+Math.sin(i)*n))return!0}return!1}function de(e,t,n,r){for(let i of S){let a=f.x[i]-e,o=f.y[i]-t,s=(h.get(i)??5)*n+r;if(a*a+o*o<s*s)return!0}return!1}function fe(e,t){return e>1.36?t<.62?`spruce`:t<.94?`pine`:`dead`:t<.58?`broadleaf`:t<.8?`birch`:t<.94?`spruce`:`dead`}function pe(e,t){let n=[];for(let r=0;r<4;r++)for(let i=0;i<4;i++){let a=e*4+i,o=t*4+r;if(Q(a,o,13122)>=.65)continue;let s=.175+Q(a,o,Z+778)*.65,c=.175+Q(a,o,Z+779)*.65,l=e*16+i*4+s*4,u=t*16+r*4+c*4,d=l+(Mt(l/8.5,u/8.5,Z+790)*2-1)*2,f=u+(Mt(l/8.5,u/8.5,Z+791)*2-1)*2;if(ue(d,f,1.5)||de(d,f,.54,.68))continue;let p=Q(a,o,Z+781)*Math.PI*2,m=.85+Q(a,o,Z+782)*.3,h=$(d,f),g=h*13,_=.05+.85*Jt(d,f),v=Q(a,o,Z+780)<_,y=1+Q(a,o,Z+785)*1.3,b=.8+Q(a,o,Z+786)*.5;if(v){let e=fe(h,Q(a,o,Z+780));e===`broadleaf`&&Q(a,o,13132)<.35&&(e=`autumn`);let t=e===`spruce`||e===`pine`?We:Ge,r=t[Math.floor(Q(a,o,Z+784)*t.length)];n.push({x:d,y:g,z:f,scale:[b,y,b],yaw:p,color:O(r,m),kind:e})}else{let e=.1+.30000000000000004*Math.min(1,h/1.6);if(Q(a,o,13140)>=e)continue;let t=Je[Math.floor(Q(a,o,Z+784)*Je.length)],r=.6+Q(a,o,Z+785)*.9,i=.6+Q(a,o,Z+786)*.9;n.push({x:d,y:g,z:f,scale:[i,r,i],yaw:p,color:O(t,m),kind:`rock`})}}for(let r=0;r<8;r++)for(let i=0;i<8;i++){let a=e*8+i,o=t*8+r;if(Q(a,o,13232)>=.7)continue;let s=Q(a,o,Z+888),c=Q(a,o,Z+889),l=e*16+i*2+s*2,u=t*16+r*2+c*2;if(ue(l,u,.4)||de(l,u,.36,.17))continue;let d=$(l,u);if(d>2)continue;let f=d*13,p=Q(a,o,Z+890)*Math.PI*2,m=.8+Q(a,o,Z+891)*.4,h=Ke[Math.floor(Q(a,o,Z+892)*Ke.length)],g=.8+Q(a,o,Z+893)*.6;n.push({x:l,y:f,z:u,scale:[g,g,g],yaw:p,color:O(h,m),kind:`grass`})}let r=16/3;for(let i=0;i<r;i++)for(let a=0;a<r;a++){let o=e*r+a,s=t*r+i;if(Q(o,s,13342)>=.35)continue;let c=Q(o,s,Z+998),l=Q(o,s,Z+999),u=e*16+a*3+c*3,d=t*16+i*3+l*3;if(ue(u,d,.9)||de(u,d,.44,.34))continue;let f=$(u,d);if(f>2)continue;let p=f*13,m=Q(o,s,Z+1e3)*Math.PI*2,h=.85+Q(o,s,Z+1001)*.3,g=qe[Math.floor(Q(o,s,Z+1002)*qe.length)],_=.9+Q(o,s,Z+1003)*.7;n.push({x:u,y:p,z:d,scale:[_,_,_],yaw:m,color:O(g,h),kind:`bush`})}return n}function k(){let e=[];for(let t of le.values())e.push(...t);D.setDecor(e),window.__decorCount=e.length,window.__decorList=e}let A=new Set,j=new Set,M=[],me=null,he=null;function ge(e,t,n=!1){let r=Math.floor(e/16),i=Math.floor(t/16);if(!n&&r===me&&i===he)return;me=r,he=i;let a=!1;for(let e=-3;e<=3;e++)for(let t=-3;t<=3;t++){let n=r+t,o=i+e,s=se(n,o);A.has(s)||j.has(s)||(j.add(s),M.push({cx:n,cz:o,key:s}),a=!0)}let o=!1;for(let e of Array.from(A)){let[t,n]=e.split(`,`).map(Number);Math.max(Math.abs(t-r),Math.abs(n-i))>5&&(D.removeTerrainChunk(e),A.delete(e),le.delete(e),o=!0)}for(let e of Array.from(j)){let[t,n]=e.split(`,`).map(Number);Math.max(Math.abs(t-r),Math.abs(n-i))>5&&(j.delete(e),a=!0)}a&&(M=M.filter(e=>j.has(e.key)),M.sort((e,t)=>(e.cx-r)**2+(e.cz-i)**2-((t.cx-r)**2+(t.cz-i)**2))),window.__terrainChunkCount=A.size,o&&k()}function _e(e){let t=!1;for(;M.length&&performance.now()<e;){let{cx:e,cz:n,key:r}=M.shift();if(!j.has(r))continue;j.delete(r);let i=e*16,a=n*16,o=nn(i,a,i+16,a+16,1);D.setTerrainChunk(r,o),A.add(r),ce(e,n),le.set(r,pe(e,n)),t=!0}t&&(window.__terrainChunkCount=A.size,k())}function ve(e,t){let n=se(e,t);if(!A.has(n))return;let r=e*16,i=t*16;D.setTerrainChunk(n,nn(r,i,r+16,i+16,1))}function ye(e,t,n,r){let i=Math.floor(n/16),a=Math.floor(r/16),o=(i-3)*16,s=(i+3+1)*16,c=(a-3)*16,l=(a+3+1)*16,u=e*64,d=t*64;return u>=o&&u+64<=s&&d>=c&&d+64<=l}let N=new Set,P=new Set,F=[],I=null,be=null;function L(e,t,n=!1){let r=Math.floor(e/64),i=Math.floor(t/64);if(!n&&r===I&&i===be)return;I=r,be=i;let a=!1;for(let n=-5;n<=5;n++)for(let o=-5;o<=5;o++){let s=r+o,c=i+n,l=`far:`+s+`,`+c;N.has(l)||P.has(l)||ye(s,c,e,t)||(P.add(l),F.push({cx:s,cz:c,rkey:l}),a=!0)}for(let n of Array.from(N)){let[a,o]=n.slice(4).split(`,`).map(Number),s=Math.max(Math.abs(a-r),Math.abs(o-i))>7,c=ye(a,o,e,t);(s||c)&&(D.removeTerrainChunk(n),N.delete(n))}for(let e of Array.from(P)){let[t,n]=e.slice(4).split(`,`).map(Number);Math.max(Math.abs(t-r),Math.abs(n-i))>7&&(P.delete(e),a=!0)}a&&(F=F.filter(e=>P.has(e.rkey)),F.sort((e,t)=>(e.cx-r)**2+(e.cz-i)**2-((t.cx-r)**2+(t.cz-i)**2))),window.__farChunkCount=N.size}function xe(e){for(;F.length&&performance.now()<e;){let{cx:e,cz:t,rkey:n}=F.shift();if(!P.has(n))continue;P.delete(n);let r=e*64,i=t*64,a=nn(r,i,r+64,i+64,4,.35);D.setTerrainChunk(n,a),N.add(n)}window.__farChunkCount=N.size}let Se=gn(C,E,D.getShadowResources()),we=new Map;function R(e){let t=we.get(e);return t||(t=pn(e).then(e=>hn(C,e)),we.set(e,t)),t}let Te=new Set(Array.from(S,e=>m.get(e)));await Promise.allSettled(Array.from(Te,e=>R(e)));let Ee=new Map,De=0,z=0;for(let t of S){let n=f.x[t],r=f.y[t],i=Ve(n,$(n,r)*13,r,0,h.get(t)??5),a=m.get(t);try{let e=await R(a);Ee.set(t,Se.createInstance(e,i)),De++}catch(t){z++,e.push(`модель: ошибка на ${a} — ${t instanceof Error?t.message:String(t)}`)}}e.push(`модели: загружено ${De}/${S.length}${z?`, ошибок: `+z:``}`),An(e),window.__ecsFound=S.length,window.__foundPositions=()=>S.map(e=>({x:f.x[e],z:f.y[e],scale:h.get(e)??5}));let B=a?a.x:n.x,Oe=a?a.y:n.y,V={yaw:0,pitch:.55,dist:42,target:[B,$(B,Oe)*13+2,Oe]},H=un(T,V);ge(V.target[0],V.target[2],!0),L(V.target[0],V.target[2],!0);let U=performance.now()+40;_e(U),xe(U),e.push(`рельеф: чанков ${A.size} (16×16) + дальних ${N.size} (64×64, шаг 4), в очереди ещё ${M.length+F.length}`),An(e),window.__coverageCheck=(e,t)=>{for(let n of A){let[r,i]=n.split(`,`).map(Number),a=r*16,o=i*16;if(e>=a&&e<a+16&&t>=o&&t<o+16)return`near`}for(let n of N){let[r,i]=n.slice(4).split(`,`).map(Number),a=r*64,o=i*64;if(e>=a&&e<a+64&&t>=o&&t<o+64)return`far`}return null},Object.defineProperty(window,"cam",{value:{get tx(){return V.target[0]},set tx(e){V.target[0]=e,H.stopAuto()},get ty(){return V.target[1]},set ty(e){V.target[1]=e,H.stopAuto()},get tz(){return V.target[2]},set tz(e){V.target[2]=e,H.stopAuto()},get dist(){return V.dist},set dist(e){V.dist=e,H.stopAuto()},get pitch(){return V.pitch},set pitch(e){V.pitch=e,H.stopAuto()}}}),window.H=(e,t)=>$(e,t)*13,window.__camState=()=>({yaw:V.yaw,pitch:V.pitch,dist:V.dist,target:[...V.target]}),window.__isAutoOrbiting=()=>H.isAutoOrbiting();let Ae=document.getElementById(`coordX`),Ne=document.getElementById(`coordY`),Pe=document.getElementById(`coordGo`),Ie=!1;for(let e of[Ae,Ne])e.addEventListener(`input`,()=>{Ie=!0});function Be(){let e=parseFloat(Ae.value),t=parseFloat(Ne.value);!isFinite(e)||!isFinite(t)||(V.target[0]=Math.max(-At,Math.min(At,e)),V.target[2]=Math.max(-600,Math.min(600,t)),V.target[1]=$(V.target[0],V.target[2])*13+2,H.stopAuto(),Ie=!1)}Pe.addEventListener(`click`,Be);for(let e of[Ae,Ne])e.addEventListener(`keydown`,t=>{t.key===`Enter`&&(t.preventDefault(),Be(),e.blur())});let G=new Float32Array(16),Ye=[0,0,0],K=document.getElementById(`selected`),q=[.95,.78,.35],J=[.42,.78,.46],Xe=[.82,.24,.26],Ze=null,Qe=null,$e=null,et=null;window.startFollowMarch=e=>{H.stopAuto(),et=e},H.onInteract(()=>{et=null});function tt(e){$e=null,Qe=e;let t=(g.get(e)??`?`)+` · `+(_.get(e)??`?`),n=f.x[e],r=f.y[e];Ze={x:n,y:$(n,r)*13+(h.get(e)??5)*.9+2,z:r,color:q},window.__markerActive=!0,window.__selectedLabel=t,K.textContent=t,K.style.display=`block`}function nt(){Qe=null,Ze=null,window.__markerActive=!1,window.__selectedLabel=null,K.style.display=`none`}function rt(e,t){let n=T.width/Math.max(1,T.height),r=Math.tan(at/2),i=e/T.width*2-1,a=1-t/T.height*2,o=W(Fe(Ye,V.target)),s=W(Le([0,1,0],o)),c=Le(o,s),l=W([i*n*r*s[0]+a*r*c[0]-o[0],i*n*r*s[1]+a*r*c[1]-o[1],i*n*r*s[2]+a*r*c[2]-o[2]]);return{origin:Ye,dir:l}}function it(e,t,n,r){let i=e[0]-n[0],a=e[1]-n[1],o=e[2]-n[2],s=i*t[0]+a*t[1]+o*t[2],c=i*i+a*a+o*o-r*r,l=s*s-c;if(l<0)return null;let u=Math.sqrt(l),d=-s-u;return d<0&&(d=-s+u),d<0?null:d}let at=.72;function ot(e,t){let n=0;for(let r=2;r<=400;r+=2){let i=e[0]+t[0]*r;if(e[1]+t[1]*r-$(i,e[2]+t[2]*r)*13<=0){let i=n,a=r;for(let n=0;n<12;n++){let n=(i+a)/2,r=e[0]+t[0]*n,o=e[2]+t[2]*n;e[1]+t[1]*n-$(r,o)*13>0?i=n:a=n}return{t:a,x:e[0]+t[0]*a,z:e[2]+t[2]*a}}n=r}return null}function st(e,t){try{let n=window.parent;n&&n!==window&&typeof n.renderCartoucheFor==`function`&&n.renderCartoucheFor(e,t)}catch{}}function ct(e){try{let t=window.parent;t&&t!==window&&typeof t.renderMarchCartoucheFor==`function`&&t.renderMarchCartoucheFor(e)}catch{}}function lt(e,t){let{origin:n,dir:r}=rt(e,t),i=null,a=1/0;for(let e of S){let t=f.x[e],o=f.y[e],s=h.get(e)??5,c=it(n,r,[t,$(t,o)*13+s*.5,o],s);c!==null&&c<a&&(a=c,i={kind:`entity`,eid:e,t:c})}for(let e of X){let t=it(n,r,[e.x,$(e.x,e.y)*13+2.2,e.y],3);t!==null&&t<a&&(a=t,i={kind:`march`,march:e,t})}let o=ot(n,r);return o!==null&&o.t<a&&(i={kind:`ground`,x:o.x,z:o.z,t:o.t},a=o.t),i}H.onTap((e,t)=>{let n=T.getBoundingClientRect(),r=lt((e-n.left)*(T.width/n.width),(t-n.top)*(T.height/n.height));if(r?.kind===`entity`){tt(r.eid);let e=y.get(r.eid);e&&st(e.x,e.y);return}if(r?.kind===`march`){nt(),$e=r.march.id,window.__selectedMarchId=r.march.id,ct(r.march.id);return}nt(),$e=null,r?.kind===`ground`&&st(Math.floor(r.x),Math.floor(r.z))});let Y=0;async function ut(){let e=Cn(V.target[0],V.target[2],448);if(!e)return;let t=new Set,n=[],r=new Set;for(let i of e){t.add(i.key);let e=b.get(i.key);if(e!==void 0){if(g.set(e,i.nm),_.set(e,i.lv),v.set(e,!!i.own),Qe===e&&tt(e),m.get(e)!==i.model){m.set(e,i.model),h.set(e,i.scale);let t=f.x[e],r=f.y[e],a=Ve(t,$(t,r)*13,r,0,i.scale);n.push(R(i.model).then(t=>void Ee.set(e,Se.createInstance(t,a))).catch(()=>{}))}continue}let a=x(i),o=$(i.x,i.y)*13,s=Ve(i.x,o,i.y,0,i.scale);n.push(R(i.model).then(e=>void Ee.set(a,Se.createInstance(e,s))).catch(()=>{})),r.add(se(Math.floor(i.x/16),Math.floor(i.y/16))),ve(Math.floor(i.x/16),Math.floor(i.y/16))}for(let[e,n]of Array.from(b))t.has(e)||(r.add(se(Math.floor(f.x[n]/16),Math.floor(f.y[n]/16))),Me(d,n),Ee.delete(n),m.delete(n),h.delete(n),g.delete(n),_.delete(n),v.delete(n),y.delete(n),b.delete(e),Qe===n&&nt());await Promise.allSettled(n),S=Array.from(Ce(d,[f,p]));let i=!1;for(let e of r){if(!A.has(e))continue;let[t,n]=e.split(`,`).map(Number);le.set(e,pe(t,n)),i=!0}i&&k(),Y++,window.__ecsFound=S.length,window.__syncCount=Y}c&&setInterval(()=>{oe()||ut().catch(e=>console.error(`live sync:`,e))},3e3);let X=[];function dt(){if(!c)return X=[],[];let e=Dn();return e?(X=e,window.__marchPositions=e,e.map(e=>({x:e.x,y:$(e.x,e.y)*13+2.2,z:e.y,color:e.own?J:Xe}))):(X=[],[])}let ft=document.getElementById(`labels`),pt=new Map,mt=1024;function ht(){let e=new Set,t=T.clientWidth,n=T.clientHeight;for(let r of S){let i=f.x[r],a=f.y[r],o=i-V.target[0],s=a-V.target[2];if(o*o+s*s>mt)continue;let c=$(i,a)*13+(h.get(r)??5)*.6+1.1,l=He(G,[i,c,a]);if(l.w<=.001)continue;let u=(l.x/l.w*.5+.5)*t,d=(1-(l.y/l.w*.5+.5))*n;if(u<-40||u>t+40||d<-40||d>n+40)continue;e.add(r);let p=pt.get(r);if(!p){let e=document.createElement(`div`);e.className=`wlabel`;let t=document.createElement(`div`);t.className=`nm`;let n=document.createElement(`div`);n.className=`lv`,e.appendChild(t),e.appendChild(n),ft.appendChild(e),p={root:e,nm:t,lv:n},pt.set(r,p)}p.nm.textContent=g.get(r)??`?`,p.nm.classList.toggle(`mine`,!!v.get(r)),p.lv.textContent=_.get(r)??``,p.root.style.transform=`translate(${u.toFixed(1)}px,${d.toFixed(1)}px) translate(-50%,-100%)`}for(let[t,n]of pt)e.has(t)||(n.root.remove(),pt.delete(t))}let gt=new Map;function _t(e,t,n,r){if(!r||!n||r<=n)return t;let i=Math.max(0,Math.min(1,(Date.now()-n)/(r-n)));return e+(t-e)*i}function vt(){let e=new Set,t=T.clientWidth,n=T.clientHeight;for(let r of X){let i=r.battle;if(!i)continue;let a=r.x-V.target[0],o=r.y-V.target[2];if(a*a+o*o>mt)continue;let s=$(r.x,r.y)*13+2.2+1.6,c=He(G,[r.x,s,r.y]);if(c.w<=.001)continue;let l=(c.x/c.w*.5+.5)*t,u=(1-(c.y/c.w*.5+.5))*n;if(l<-60||l>t+60||u<-60||u>n+60)continue;e.add(r.id);let d=gt.get(r.id);if(!d){let e=document.createElement(`div`);e.className=`blabel`;let t=document.createElement(`div`);t.className=`btitle`;let n=document.createElement(`div`);n.className=`bbar atk`;let i=document.createElement(`i`);n.appendChild(i);let a=document.createElement(`div`);a.className=`bbar def`;let o=document.createElement(`i`);a.appendChild(o),e.appendChild(t),e.appendChild(n),e.appendChild(a),ft.appendChild(e),d={root:e,title:t,atkFill:i,defFill:o},gt.set(r.id,d)}let f=i.retreating,p=!f&&i.revealFromRound===0;d.root.className=`blabel`+(f?` retreat`:p?` deploy`:``),d.title.textContent=f?`Отступление`:p?`Развёртывание`:`Бой — раунд `+i.round;let m=Math.max(0,Math.min(100,_t(i.revealFromAttHp,i.attHpLeft,i.revealStart,i.revealAt)/Math.max(1,i.attStartHp)*100)),h=Math.max(0,Math.min(100,_t(i.revealFromDefHp,i.defHpLeft,i.revealStart,i.revealAt)/Math.max(1,i.defStartHp)*100));d.atkFill.style.width=m.toFixed(1)+`%`,d.defFill.style.width=h.toFixed(1)+`%`,d.root.style.transform=`translate(${l.toFixed(1)}px,${u.toFixed(1)}px) translate(-50%,-100%)`}for(let[t,n]of gt)e.has(t)||(n.root.remove(),gt.delete(t))}function yt(e){if(oe()){requestAnimationFrame(yt);return}H.isAutoOrbiting()&&(V.yaw=e*15e-5),H.update(e);let t=dt();if(et!==null){let e=X.find(e=>e.id===et);e?(V.target[0]=e.x,V.target[2]=e.y,V.target[1]=$(e.x,e.y)*13+1):et=null}Ie||(Ae.value=V.target[0].toFixed(1),Ne.value=V.target[2].toFixed(1)),ge(V.target[0],V.target[2]),L(V.target[0],V.target[2]);let n=performance.now()+6;_e(n),xe(n);let a=[V.target[0]+Math.sin(V.yaw)*Math.cos(V.pitch)*V.dist,V.target[1]+Math.sin(V.pitch)*V.dist,V.target[2]+Math.cos(V.yaw)*Math.cos(V.pitch)*V.dist],o=$(a[0],a[2])*13+2;a[1]<o&&(a[1]=o);let s=T.width/Math.max(1,T.height),c=Re(ze(at,s,.5,300),Ue(a,V.target,[0,1,0]));G=c,Ye=a,D.setVP(c),D.setFog(a,r,i,e/1e3),D.setSunTarget(V.target[0],V.target[2]);{let t=W(Fe(a,V.target)),n=W(Le([0,1,0],t)),r=Le(t,n);D.setSkyCamera(n,r,t,Math.tan(at/2),s,e/1e3)}if(Se.setFog(a,r,i),$e!==null){let e=X.find(e=>e.id===$e);e?Ze={x:e.x,y:$(e.x,e.y)*13+3.2,z:e.y,color:q}:($e=null,Ze=null)}Ze&&t.push(Ze),D.setMarkers(t),window.__marchCount=t.length-+!!Ze,D.frame({r:r[0],g:r[1],b:r[2],a:1},e=>{for(let t of S){let n=Ee.get(t);n&&Se.draw(e,n,c)}}),ht(),vt(),requestAnimationFrame(yt)}requestAnimationFrame(yt),window.__engineReady=!0}Mn().catch(e=>{jn([`Ошибка: ${e instanceof Error?e.message:String(e)}`]),console.error(e)});