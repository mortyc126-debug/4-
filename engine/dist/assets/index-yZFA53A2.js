...
fn fs(in: VOut) -> @location(0) vec4f {
  ...
  // неосвещённым цветом.
  //
  // Раньше тут была ОДНА ТОЛЬКО линия границы. Автор сообщил: «полупрозрачная
  // разметка на территории как бы уже наложена на карту, но я её почему-то не
  // вижу» — и был прав по сути: регионов шестнадцать на весь мир 2400×1200,
  // каждый около 380×380 клеток, а камера охватывает полсотни. По замеру
  // прежней текстуры в кадре вокруг (0,0) линия занимала 16% площади, а
  // вокруг (200,-150) и (-400,-200) — ровно 0%. Из середины региона границу
  // нельзя было увидеть в принципе. Теперь текстура несёт ещё и слабую
  // заливку территории своим цветом (см. bake_regions_overlay.py) — она
  // читается в любой точке, а граница стала сменой цвета, заметной даже краем
  // кадра.
  //
...
`,Ot=.5,kt=1.4,At=new Float32Array([0,kt,0,Ot,0,0,0,0,Ot,0,kt,0,0,0,Ot,-.5,0,0,0,kt,0,-.5,0,0,0,0,-.5,0,kt,0,0,0,-.5,Ot,0,0]),jt=At.length/3,q=7;async function Mt(e,t,n){let r=e.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),i=e.createBuffer({size:32,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),a=e.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),o=e.createTexture({size:[_t,_t],format:`depth32float`,usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.TEXTURE_BINDING}).createView(),s=e.createSampler({compare:`less`,magFilter:`linear`,minFilter:`linear`}),c=He(-1,1,-1,1,.1,1),l=!0,u=1/0,d=1/0,f=1.5;function p(t,n){let r=t-u,i=n-d;if(!l&&r*r+i*i<f*f)return;u=t,d=n;let o=qe([t+gt[0]*yt,gt[1]*yt,n+gt[2]*yt],[t,0,n],[0,1,0]);c=Be(He(-60,vt,-60,vt,bt,xt),o),e.queue.writeBuffer(a,0,c),l=!0}let[m,h,g,_,v,y,b,x,S,ee,C,w]=await Promise.all([K(e,`/textures/ground/sand.jpg`),K(e,`/textures/ground/grass.jpg`),K(e,`/textures/ground/dry_meadow.jpg`),K(e,`/textures/ground/scree.jpg`),K(e,`/textures/ground/rock.jpg`),K(e,`/textures/ground/snow.jpg`),K(e,`/textures/ground/forest_floor.jpg`),K(e,`/textures/ground/desert.jpg`),K(e,`/textures/ground/marsh.jpg`),K(e,`/textures/ground/tundra_moss.jpg`),K(e,`/textures/water/detail.jpg`),K(e,`/textures/world/regions_overlay.png`,2400)]),te=e.createSampler({addressModeU:`repeat`,addressModeV:`repeat`,magFilter:`linear`,minFilter:`linear`}),ne=e.createShaderModule({code:St}),re=e.createRenderPipeline({layout:`auto`,vertex:{module:ne,entryPoint:`vs`,buffers:[{arrayStride:60,attributes:[{shaderLocation:0,offset:0,format:`float32x3`},{shaderLocation:1,offset:12,format:`float32x3`},{shaderLocation:2,offset:24,format:`float32x3`},{shaderLocation:3,offset:36,format:`float32x2`},{shaderLocation:4,offset:44,format:`float32`},{shaderLocation:5,offset:48,format:`float32`},{shaderLocation:6,offset:52,format:`float32`},{shaderLocation:7,offset:56,format:`float32`}]}]},fragment:{module:ne,entryPoint:`fs`,targets:[{format:n}]},primitive:{topology:`triangle-list`,cullMode:`back`},depthStencil:{format:`depth24plus`,depthWriteEnabled:!0,depthCompare:`less`}}),ie=e.createBindGroup({layout:re.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:r}},{binding:1,resource:{buffer:i}},{binding:2,resource:te},{binding:3,resource:m.createView()},{binding:4,resource:h.createView()},{binding:5,resource:g.createView()},{binding:6,resource:_.createView()},{binding:7,resource:v.createView()},{binding:8,resource:{buffer:a}},{binding:9,resource:s},{binding:10,resource:o},{binding:11,resource:y.createView()},{binding:12,resource:b.createView()},{binding:13,resource:x.createView()},{binding:14,resource:S.createView()},{binding:15,resource:ee.createView()},{binding:16,resource:C.createView()},{binding:17,resource:w.createView()}]}),[ae,T]=await Promise.all([K(e,`/textures/sky/sky.jpg`),K(e,`/textures/sky/clouds.png`)]),oe=e.createSampler({addressModeU:`repeat`,addressModeV:`clamp-to-edge`,magFilter:`linear`,minFilter:`linear`}),se=e.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),ce=e.createShaderModule({code:Dt}),le=e.createRenderPipeline({layout:`auto`,vertex:{module:ce,entryPoint:`vs`},fragment:{module:ce,entryPoint:`fs`,targets:[{format:n}]},primitive:{topology:`triangle-list`,cullMode:`none`},depthStencil:{format:`depth24plus`,depthWriteEnabled:!1,depthCompare:`always`}}),E=e.createBindGroup({layout:le.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:se}},{binding:1,resource:oe},{binding:2,resource:ae.createView()},{binding:3,resource:T.createView()}]}),ue=new Float32Array(16);function de(t,n,r,i,a,o){let s=ue;s[0]=t[0],s[1]=t[1],s[2]=t[2],s[3]=0,s[4]=n[0],s[5]=n[1],s[6]=n[2],s[7]=0,s[8]=r[0],s[9]=r[1],s[10]=r[2],s[11]=0,s[12]=i,s[13]=a,s[14]=o,s[15]=0,e.queue.writeBuffer(se,0,s)}let fe=e.createShaderModule({code:Tt}),pe=e.createRenderPipeline({layout:`auto`,vertex:{module:fe,entryPoint:`vs`,buffers:[{arrayStride:60,attributes:[{shaderLocation:0,offset:0,format:`float32x3`}]}]},primitive:{topology:`triangle-list`,cullMode:`back`},depthStencil:{format:`depth32float`,depthWriteEnabled:!0,depthCompare:`less`}}),me=e.createBindGroup({layout:pe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:a}}]}),D=new Set;function he(e){return e===`world-backdrop`?`backdrop`:e.startsWith(`far:`)?`far`:`near`}let O={near:null,far:null,backdrop:null},ge=t=>new ht({createBuffer(n){O[t]?.destroy(),O[t]=e.createBuffer({size:n,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST})},write(n,r){let i=O[t];i&&e.queue.writeBuffer(i,n,r,0,r.length)}}),_e={near:ge(`near`),far:ge(`far`),backdrop:ge(`backdrop`)},ve=e.createShaderModule({code:Ct}),ye=e.createBuffer({size:At.byteLength,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST});e.queue.writeBuffer(ye,0,At);let k=e.createRenderPipeline({layout:`auto`,vertex:{module:ve,entryPoint:`vs`,buffers:[{arrayStride:12,stepMode:`vertex`,attributes:[{shaderLocation:0,offset:0,format:`float32x3`}]},{arrayStride:28,stepMode:`instance`,attributes:[{shaderLocation:1,offset:0,format:`float32x3`},{shaderLocation:2,offset:12,format:`float32`},{shaderLocation:3,offset:16,format:`float32x3`}]}]},fragment:{module:ve,entryPoint:`fs`,targets:[{format:n}]},primitive:{topology:`triangle-list`},depthStencil:{format:`depth24plus`,depthWriteEnabled:!0,depthCompare:`less`}}),be=e.createBindGroup({layout:k.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:r}},{binding:1,resource:{buffer:i}}]}),A=null,xe=null,Se=0,j=0,Ce=e.createShaderModule({code:wt});function we(t){let n=e.createBuffer({size:Math.max(t.vertexCount*10*4,4),usage:GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST}),r=new Float32Array(t.vertexCount*10);for(let e=0;e<t.vertexCount;e++)r.set(t.positions.subarray(e*3,e*3+3),e*10),r.set(t.normals.subarray(e*3,e*3+3),e*10+3),r[e*10+6]=t.materialIds[e],r[e*10+7]=t.shades[e],r.set(t.uvs.subarray(e*2,e*2+2),e*10+8);return e.queue.writeBuffer(n,0,r),n}let Te=await Promise.all(Object.entries({bark:`/textures/decor/bark.jpg`,birchBark:`/textures/decor/birch_bark.jpg`,conifer:`/textures/decor/conifer_a.png`,conifer2:`/textures/decor/conifer_b.png`,broadleaf:`/textures/decor/broadleaf.png`,autumn:`/textures/decor/autumn.png`,birchLeaf:`/textures/decor/birch_leaf.png`,bush:`/textures/decor/bush.png`,grassTuft:`/textures/decor/grass_tuft.png`}).map(async([t,n])=>[t,await K(e,n)])),M={...Object.fromEntries(Te),rock:v},N=e.createSampler({magFilter:`linear`,minFilter:`linear`}),P={spruce:{trunk:`bark`,canopy:`conifer`},pine:{trunk:`bark`,canopy:`conifer2`},broadleaf:{trunk:`bark`,canopy:`broadleaf`},autumn:{trunk:`bark`,canopy:`autumn`},birch:{trunk:`birchBark`,canopy:`birchLeaf`},dead:{trunk:`bark`,canopy:`bark`},bush:{trunk:`bark`,canopy:`bush`},grass:{trunk:`bark`,canopy:`grassTuft`},rock:{trunk:`bark`,canopy:`rock`}},Ee={spruce:nt,pine:rt,broadleaf:G,autumn:G,birch:it,dead:at,bush:ot,grass:st,rock:mt},De=[{arrayStride:40,stepMode:`vertex`,attributes:[{shaderLocation:0,offset:0,format:`float32x3`},{shaderLocation:1,offset:12,format:`float32x3`},{shaderLocation:2,offset:24,format:`float32`},{shaderLocation:3,offset:28,format:`float32`},{shaderLocation:4,offset:32,format:`float32x2`}]},{arrayStride:40,stepMode:`instance`,attributes:[{shaderLocation:5,offset:0,format:`float32x3`},{shaderLocation:6,offset:12,format:`float32x3`},{shaderLocation:7,offset:24,format:`float32`},{shaderLocation:8,offset:28,format:`float32x3`}]}],Oe=e.createRenderPipeline({layout:`auto`,vertex:{module:Ce,entryPoint:`vs`,buffers:De},fragment:{module:Ce,entryPoint:`fs`,targets:[{format:n}]},primitive:{topology:`triangle-list`},depthStencil:{format:`depth24plus`,depthWriteEnabled:!0,depthCompare:`less`}}),ke=e.createShaderModule({code:Et}),Ae=e.createRenderPipeline({layout:`auto`,vertex:{module:ke,entryPoint:`vs`,buffers:De},fragment:{module:ke,entryPoint:`fs`,targets:[]},primitive:{topology:`triangle-list`},depthStencil:{format:`depth32float`,depthWriteEnabled:!0,depthCompare:`less`}}),F=new Map;for(let t of Object.keys(P)){let n=Ee[t](),c=P[t],l=e.createBindGroup({layout:Oe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:r}},{binding:1,resource:{buffer:i}},{binding:2,resource:N},{binding:3,resource:M[c.trunk].createView()},{binding:4,resource:M[c.canopy].createView()},{binding:5,resource:{buffer:a}},{binding:6,resource:s},{binding:7,resource:o}]}),u=e.createBindGroup({layout:Ae.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:a}},{binding:1,resource:N},{binding:2,resource:M[c.canopy].createView()}]});F.set(t,{mesh:n,localBuf:we(n),instBuf:null,instCapacity:0,instanceCount:0,bindGroup:l,shadowBindGroup:u,scratch:null,shadowInstBuf:null,shadowInstCapacity:0,shadowInstanceCount:0,shadowScratch:null})}let I=null,L=null;function je(){let n=t.canvas.width,r=t.canvas.height;I&&I.width===n&&I.height===r||(I?.destroy(),I=e.createTexture({size:[n,r],format:`depth24plus`,usage:GPUTextureUsage.RENDER_ATTACHMENT}),L=I.createView())}function R(e,t){let n=new Float32Array(t.vertexCount*15);for(let e=0;e<t.vertexCount;e++)n.set(t.positions.subarray(e*3,e*3+3),e*15),n.set(t.colors.subarray(e*3,e*3+3),e*15+3),n.set(t.normals.subarray(e*3,e*3+3),e*15+6),n.set(t.uvs.subarray(e*2,e*2+2),e*15+9),n[e*15+11]=t.elevations[e],n[e*15+12]=t.waterFlags[e],n[e*15+13]=t.forestFracs[e],n[e*15+14]=t.moistureFracs[e];D.add(e);let r=he(e);_e[r].put(e,n,t.vertexCount),r===`near`&&(l=!0)}function Me(e){if(!D.has(e))return;D.delete(e);let t=he(e);_e[t].remove(e),t===`near`&&(l=!0)}function Ne(t){if(j=t.length,j>Se&&(A?.destroy(),Se=Math.max(j,8),A=e.createBuffer({size:Se*q*4,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST}),xe=new Float32Array(Se*q)),j===0||!A)return;let n=xe;t.forEach((e,t)=>{let r=t*q;n[r]=e.x,n[r+1]=e.y,n[r+2]=e.z,n[r+3]=1,n[r+4]=e.color[0],n[r+5]=e.color[1],n[r+6]=e.color[2]}),e.queue.writeBuffer(A,0,n,0,j*q)}function z(t,n,r){let i=t.scratch;if(!i||t.instanceCount===0){t.shadowInstanceCount=0;return}let a=n-vt,o=n+vt,s=r-vt,c=r+vt;t.instanceCount>t.shadowInstCapacity&&(t.shadowInstBuf?.destroy(),t.shadowInstCapacity=Math.max(t.instanceCount,8),t.shadowInstBuf=e.createBuffer({size:t.shadowInstCapacity*10*4,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST}),t.shadowScratch=new Float32Array(t.shadowInstCapacity*10));let l=t.shadowScratch,u=0;for(let e=0;e<t.instanceCount;e++){let t=e*10,n=i[t],r=i[t+2];n<a||n>o||r<s||r>c||(l.set(i.subarray(t,t+10),u*10),u++)}t.shadowInstanceCount=u,u>0&&t.shadowInstBuf&&e.queue.writeBuffer(t.shadowInstBuf,0,l,0,u*10)}function Pe(t,n){let r=t.length;if(r>n.instCapacity&&(n.instBuf?.destroy(),n.instCapacity=Math.max(r,8),n.instBuf=e.createBuffer({size:n.instCapacity*10*4,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST}),n.scratch=new Float32Array(n.instCapacity*10)),n.instanceCount=r,r===0||!n.instBuf)return;let i=n.scratch;t.forEach((e,t)=>{let n=t*10;i[n]=e.x,i[n+1]=e.y,i[n+2]=e.z,i[n+3]=e.scale[0],i[n+4]=e.scale[1],i[n+5]=e.scale[2],i[n+6]=e.yaw,i[n+7]=e.color[0],i[n+8]=e.color[1],i[n+9]=e.color[2]}),e.queue.writeBuffer(n.instBuf,0,i,0,r*10)}function Fe(e){let t=new Map;for(let n of e){let e=t.get(n.kind);e||(e=[],t.set(n.kind,e)),e.push(n)}for(let[e,n]of F)Pe(t.get(e)??[],n);l=!0}function Ie(t){e.queue.writeBuffer(r,0,t)}let Le=new Float32Array(8);function B(t,n,r,a){let o=Le;o[0]=t[0],o[1]=t[1],o[2]=t[2],o[3]=a,o[4]=n[0],o[5]=n[1],o[6]=n[2],o[7]=r,e.queue.writeBuffer(i,0,o)}function Re(n,r){je();let i=e.createCommandEncoder();if(l){let e=i.beginRenderPass({colorAttachments:[],depthStencilAttachment:{view:o,depthClearValue:1,depthLoadOp:`clear`,depthStoreOp:`store`}});_e.near.vertexCount>0&&O.near&&(e.setPipeline(pe),e.setBindGroup(0,me),e.setVertexBuffer(0,O.near),e.draw(_e.near.vertexCount));let t=!1;for(let e of F.values()){if(e.instanceCount===0){e.shadowInstanceCount=0;continue}z(e,u,d),e.shadowInstanceCount>0&&(t=!0)}if(t){e.setPipeline(Ae);for(let t of F.values())t.shadowInstanceCount===0||!t.shadowInstBuf||(e.setBindGroup(0,t.shadowBindGroup),e.setVertexBuffer(0,t.localBuf),e.setVertexBuffer(1,t.shadowInstBuf),e.draw(t.mesh.vertexCount,t.shadowInstanceCount))}e.end(),l=!1}let a=t.getCurrentTexture().createView(),s=i.beginRenderPass({colorAttachments:[{view:a,clearValue:n,loadOp:`clear`,storeOp:`store`}],depthStencilAttachment:{view:L,depthClearValue:1,depthLoadOp:`clear`,depthStoreOp:`store`}});if(s.setPipeline(le),s.setBindGroup(0,E),s.draw(3),D.size>0){s.setPipeline(re),s.setBindGroup(0,ie);for(let e of[`near`,`far`,`backdrop`]){let t=_e[e],n=O[e];t.vertexCount===0||!n||(s.setVertexBuffer(0,n),s.draw(t.vertexCount))}}j>0&&A&&(s.setPipeline(k),s.setBindGroup(0,be),s.setVertexBuffer(0,ye),s.setVertexBuffer(1,A),s.draw(jt,j));let c=!1;for(let e of F.values())if(e.instanceCount>0){c=!0;break}if(c){s.setPipeline(Oe);for(let e of F.values())e.instanceCount===0||!e.instBuf||(s.setBindGroup(0,e.bindGroup),s.setVertexBuffer(0,e.localBuf),s.setVertexBuffer(1,e.instBuf),s.draw(e.mesh.vertexCount,e.instanceCount))}r?.(s),s.end(),e.queue.submit([i.finish()])}function V(){return{lightBuf:a,shadowView:o,shadowSampler:s}}return{setTerrainChunk:R,removeTerrainChunk:Me,setMarkers:Ne,setDecor:Fe,setVP:Ie,setFog:B,setSunTarget:p,setSkyCamera:de,getShadowResources:V,frame:Re}}var J=12345,Nt=.235,Pt=2400,Ft=1200,It=Pt/2;Ft/2;var Lt=2.5;function Y(e,t,n){let r=e*374761393+t*668265263+n*1274126177;return r=Math.imul(r^r>>>13,1274126177),((r^r>>>16)>>>0)/4294967296}function Rt(e,t,n){let r=Math.floor(e),i=Math.floor(t),a=e-r,o=t-i,s=a*a*(3-2*a),c=o*o*(3-2*o),l=Y(r,i,n),u=Y(r+1,i,n),d=Y(r,i+1,n),f=Y(r+1,i+1,n);return(l*(1-s)+u*s)*(1-c)+(d*(1-s)+f*s)*c}var zt=null,X=null,Bt=null;async function Vt(e,t){let n=await fetch(e);if(!n.ok)throw Error(`${e}: HTTP ${n.status}`);let r=await n.arrayBuffer();if(r.byteLength!==t)throw Error(`${e}: неверный размер (${r.byteLength} байт, ожидалось ${t})`);return r}async function Ht(){let e=Pt*Ft,[t,n,r]=await Promise.all([Vt(`/heightmap/elevation-v6.bin`,e*2),Vt(`/heightmap/forest.bin`,e),Vt(`/heightmap/moisture.bin`,e)]);zt=new Uint16Array(t),X=new Uint8Array(n),Bt=new Uint8Array(r)}function Ut(e,t,n,r){let i=Math.floor(t),a=Math.floor(n),o=Math.min(i+1,2399),s=Math.min(a+1,1199),c=t-i,l=n-a,u=a*Pt+i,d=a*Pt+o,f=s*Pt+i,p=s*Pt+o,m=e[u]+(e[d]-e[u])*c;return(m+(e[f]+(e[p]-e[f])*c-m)*l)*r}function Wt(e,t){return[Math.max(0,Math.min(2399,e+It)),Math.max(0,Math.min(1199,t+600))]}function Gt(e,t){if(!zt)return .285;let[n,r]=Wt(e,t);return Ut(zt,n,r,Lt/65535)}function Kt(e,t){let n=Gt(e,t),r=(Gt(e+.7,t)+Gt(e-.7,t)+Gt(e,t+.7)+Gt(e,t-.7))*.25;return n*.55+r*.45}var qt=32,Jt=new Map;function Yt(e,t){return Math.floor(e/qt)+`,`+Math.floor(t/qt)}function Xt(e,t,n){let r={x:e,z:t,targetH:Math.max(Kt(e,t),.245),radius:n},i=Yt(e,t),a=Jt.get(i);a?a.push(r):Jt.set(i,[r])}function Z(e,t){let n=Gt(e,t);if(n<.235||Jt.size===0)return n;let r=Math.floor(e/qt),i=Math.floor(t/qt),a=0,o=0;for(let n=-1;n<=1;n++)for(let s=-1;s<=1;s++){let c=Jt.get(r+s+`,`+(i+n));if(c)for(let n of c){let r=Math.hypot(e-n.x,t-n.z);if(r>=n.radius)continue;let i=n.radius*.55,s=r<=i?1:1-((r-i)/(n.radius-i))**2*(3-2*((r-i)/(n.radius-i)));a+=s,o+=s*n.targetH}}return a<=0?n:a>=1?o/a:n*(1-a)+o}function Zt(e,t){return Z(e,t)<Nt}function Qt(e,t){if(!Bt)return .5;let[n,r]=Wt(e,t);return Ut(Bt,n,r,1/255)}function $t(e,t){if(!X)return 0;let[n,r]=Wt(e,t);return Ut(X,n,r,1/255)}var en=(e,t,n)=>[e[0]+(t[0]-e[0])*n,e[1]+(t[1]-e[1])*n,e[2]+(t[2]-e[2])*n],Q=[.14,.24,.28],tn=[.05,.11,.19];function nn(e){return en(Q,tn,Math.min(1,e))}var rn=[0,1,0],an=6;function on(e,t){let n=.5,r=Z(e-n,t)*13,i=Z(e+n,t)*13,a=Z(e,t-n)*13,o=Z(e,t+n)*13;return H([-(i-r)/(2*n),1,-(o-a)/(2*n)])}var sn=6;function cn(e,t,n){let r=n/2,i=1/0;for(let a=0;a<=sn;a++){let o=-r+a/sn*n;for(let a=0;a<=sn;a++){let s=Z(e+(-r+a/sn*n),t+o);s<i&&(i=s)}}return i}function ln(e,t,n,r,i=1,a=0){let o=Math.round((n-e)/i),s=Math.round((r-t)/i),c=i===1,l=[],u=[],d=[],f=[],p=[],m=[],h=[],g=[];function _(e,t){let n=c?Z(e,t):cn(e,t,i),r=n<Nt,o=r?[e,Nt*13-a,t]:[e,n*13-a,t],s=r?nn((Nt-n)*3):[0,0,0],l=r?rn:c?on(e,t):rn,u=r?0:$t(e,t),d=r?0:Qt(e,t);return{p:o,c:s,n:l,uv:[e/an,t/an],e:n,water:+!!r,forest:u,moisture:d}}let v=[];for(let n=0;n<=s;n++){let r=[];for(let a=0;a<=o;a++)r.push(_(e+a*i,t+n*i));v.push(r)}function y(e,t,n){let r=c?null:H(ze(Re(t.p,e.p),Re(n.p,e.p)));for(let i of[e,t,n]){l.push(i.p[0],i.p[1],i.p[2]),u.push(i.c[0],i.c[1],i.c[2]);let e=r??i.n;d.push(e[0],e[1],e[2]),f.push(i.uv[0],i.uv[1]),p.push(i.e),m.push(i.water),h.push(i.forest),g.push(i.moisture)}}for(let e=0;e<s;e++)for(let t=0;t<o;t++){let n=v[e][t],r=v[e][t+1],i=v[e+1][t],a=v[e+1][t+1];y(n,a,r),y(n,i,a)}return{positions:new Float32Array(l),colors:new Float32Array(u),normals:new Float32Array(d),uvs:new Float32Array(f),elevations:new Float32Array(p),waterFlags:new Float32Array(m),forestFracs:new Float32Array(h),moistureFracs:new Float32Array(g),vertexCount:l.length/3}}var un=9,dn=10,fn=380,pn={d:[1,0],arrowright:[1,0],a:[-1,0],arrowleft:[-1,0],w:[0,1],arrowup:[0,1],s:[0,-1],arrowdown:[0,-1]},mn=700;function hn(e,t){let n=!0,r=new Map,i=null,a=null,o=null,s=null,c=null,l=null,u=null,d=!1,f=null;function p(){n=!1,f?.()}function m(){let e=[...r.values()];return{x:(e[0].x+e[1].x)/2,y:(e[0].y+e[1].y)/2,d:Math.hypot(e[0].x-e[1].x,e[0].y-e[1].y)}}function h(){let e=[...r.values()];return Math.atan2(e[1].y-e[0].y,e[1].x-e[0].x)}function g(e,n){let r=t.dist*.0022,i=e*r,a=n*r,o=Math.cos(t.yaw),s=Math.sin(t.yaw);t.target[0]=Math.max(-It,Math.min(It,t.target[0]-(i*o-a*s))),t.target[2]=Math.max(-600,Math.min(600,t.target[2]+(i*s+a*o))),t.target[1]=Z(t.target[0],t.target[2])*13+1}e.addEventListener(`pointerdown`,n=>{n.preventDefault(),p(),r.set(n.pointerId,{x:n.clientX,y:n.clientY});try{e.setPointerCapture(n.pointerId)}catch{}if(r.size===1)d=!!c?.(n.clientX,n.clientY),d?(i=null,o={x:n.clientX,y:n.clientY,t:performance.now()}):(i={x:n.clientX,y:n.clientY,tx:t.target[0],tz:t.target[2]},o={x:n.clientX,y:n.clientY,t:performance.now()});else if(r.size===2){d&&(d=!1,u?.(NaN,NaN)),i=null,o=null;let e=m();a={d:e.d,y:e.y,dist:t.dist,yaw:t.yaw,pitch:t.pitch,angle:h()}}}),e.addEventListener(`pointermove`,e=>{if(r.has(e.pointerId)){if(e.preventDefault(),r.set(e.pointerId,{x:e.clientX,y:e.clientY}),o&&Math.hypot(e.clientX-o.x,e.clientY-o.y)>dn&&(o=null),d){l?.(e.clientX,e.clientY);return}if(r.size>=2&&a){let e=m();t.dist=Math.max(un,Math.min(140,a.dist*(a.d/Math.max(12,e.d)))),t.yaw=a.yaw+(h()-a.angle),t.pitch=Math.max(.08,Math.min(1.42,a.pitch+(e.y-a.y)*.005));return}i&&(t.target[0]=i.tx,t.target[2]=i.tz,g(e.clientX-i.x,i.y-e.clientY))}});function _(e){let n=!!(o&&r.size===1&&performance.now()-o.t<fn);if(d&&(d=!1,u?.(n?NaN:e.clientX,n?NaN:e.clientY)),n&&s?.(o.x,o.y),o=null,r.delete(e.pointerId),r.size<2&&(a=null),r.size===0)i=null;else if(r.size===1){let e=[...r.values()][0];i={x:e.x,y:e.y,tx:t.target[0],tz:t.target[2]}}}e.addEventListener(`pointerup`,_),e.addEventListener(`pointercancel`,_),e.addEventListener(`wheel`,e=>{e.preventDefault(),p(),t.dist=Math.max(un,Math.min(140,t.dist*(e.deltaY<0?.9:1.11)))},{passive:!1});let v=new Set;window.addEventListener(`keydown`,e=>{let t=e.key.toLowerCase();t in pn&&(v.add(t),p())}),window.addEventListener(`keyup`,e=>{v.delete(e.key.toLowerCase())});let y=null;function b(e){if(y===null){y=e;return}let t=Math.min(.1,(e-y)/1e3);if(y=e,v.size===0||i)return;let n=0,r=0;for(let e of v){let[t,i]=pn[e];n+=t,r+=i}(n!==0||r!==0)&&g(n*mn*t,r*mn*t)}return{isAutoOrbiting:()=>n,stopAuto:p,update:b,onGrab(e,t,n){c=e,l=t,u=n},onTap(e){s=e},onInteract(e){f=e}}}var gn={5121:Uint8Array,5123:Uint16Array,5125:Uint32Array,5126:Float32Array},_n={SCALAR:1,VEC2:2,VEC3:3,VEC4:4};async function vn(e){let t=await(await fetch(e)).arrayBuffer(),n=new DataView(t);if(n.getUint32(0,!0)!==1179937895)throw Error(`не glTF-контейнер: `+e);let r=n.getUint32(8,!0),i=12,a=null,o=null;for(;i<r;){let e=n.getUint32(i,!0),r=n.getUint32(i+4,!0),s=t.slice(i+8,i+8+e);r===1313821514?a=JSON.parse(new TextDecoder().decode(s)):r===5130562&&(o=s),i+=8+e}if(!a||!o)throw Error(`GLB без JSON/BIN чанка: `+e);let s=e=>a.accessors[e],c=e=>a.bufferViews[e];function l(e){let t=s(e),n=c(t.bufferView),r=gn[t.componentType],i=(n.byteOffset||0)+(t.byteOffset||0),a=_n[t.type],l=a*r.BYTES_PER_ELEMENT,u=n.byteStride||0;if(!u||u===l)return new r(o,i,t.count*a);let d=new r(t.count*a);for(let e=0;e<t.count;e++)d.set(new r(o,i+e*u,a),e*a);return d}let u=a.meshes[0].primitives[0],d=l(u.attributes.POSITION),f=l(u.attributes.NORMAL),p=l(u.attributes.TEXCOORD_0),m=l(u.indices),h=a.materials[u.material].pbrMetallicRoughness.baseColorTexture.index,g=a.images[a.textures[h].source],_=c(g.bufferView);return{positions:d,normals:f,uvs:p,indices:m,imageBytes:o.slice(_.byteOffset||0,(_.byteOffset||0)+_.byteLength),imageMimeType:g.mimeType}}var yn=`
...
`;async function bn(e,t){let n=e.createBuffer({size:t.positions.byteLength,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST});e.queue.writeBuffer(n,0,t.positions);let r=e.createBuffer({size:t.normals.byteLength,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST});e.queue.writeBuffer(r,0,t.normals);let i=e.createBuffer({size:t.uvs.byteLength,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST});e.queue.writeBuffer(i,0,t.uvs);let a=t.indices.byteLength,o=Math.ceil(a/4)*4,s=e.createBuffer({size:o,usage:GPUBufferUsage.INDEX|GPUBufferUsage.COPY_DST});if(o===a)e.queue.writeBuffer(s,0,t.indices);else{let n=new Uint8Array(o);n.set(new Uint8Array(t.indices.buffer,t.indices.byteOffset,a)),e.queue.writeBuffer(s,0,n)}let c=await createImageBitmap(new Blob([t.imageBytes],{type:t.imageMimeType})),l=Math.min(1,1024/Math.max(c.width,c.height)),u=l<1?await createImageBitmap(c,{resizeWidth:Math.round(c.width*l),resizeHeight:Math.round(c.height*l),resizeQuality:`medium`}):c;l<1&&c.close();let d=e.createTexture({size:[u.width,u.height],format:`rgba8unorm`,usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_DST|GPUTextureUsage.RENDER_ATTACHMENT});return e.queue.copyExternalImageToTexture({source:u},{texture:d},[u.width,u.height]),u.close(),{vao:{posBuf:n,nrmBuf:r,uvBuf:i,idxBuf:s,indexFormat:t.indices instanceof Uint16Array?`uint16`:`uint32`,indexCount:t.indices.length},texture:d}}function xn(e,t,n){let r=e.createShaderModule({code:yn}),i=e.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}},{binding:1,visibility:GPUShaderStage.FRAGMENT,sampler:{type:`filtering`}},{binding:2,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:`float`}},{binding:3,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}},{binding:4,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}},{binding:5,visibility:GPUShaderStage.FRAGMENT,sampler:{type:`comparison`}},{binding:6,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:`depth`}},{binding:7,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}},{binding:8,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:`uniform`}}]}),a=e.createPipelineLayout({bindGroupLayouts:[i]}),o=e.createRenderPipeline({layout:a,vertex:{module:r,entryPoint:`vs`,buffers:[{arrayStride:12,attributes:[{shaderLocation:0,offset:0,format:`float32x3`}]},{arrayStride:12,attributes:[{shaderLocation:1,offset:0,format:`float32x3`}]},{arrayStride:8,attributes:[{shaderLocation:2,offset:0,format:`float32x2`}]}]},fragment:{module:r,entryPoint:`fs`,targets:[{format:t}]},primitive:{topology:`triangle-list`,cullMode:`back`},depthStencil:{format:`depth24plus`,depthWriteEnabled:!0,depthCompare:`less`}}),s=e.createRenderPipeline({layout:a,vertex:{module:r,entryPoint:`vsOutline`,buffers:[{arrayStride:12,attributes:[{shaderLocation:0,offset:0,format:`float32x3`}]},{arrayStride:12,attributes:[{shaderLocation:1,offset:0,format:`float32x3`}]}]},fragment:{module:r,entryPoint:`fsOutline`,targets:[{format:t}]},primitive:{topology:`triangle-list`,cullMode:`front`},depthStencil:{format:`depth24plus`,depthWriteEnabled:!1,depthCompare:`less`}}),c=e.createSampler({magFilter:`linear`,minFilter:`linear`,mipmapFilter:`linear`}),l=e.createBuffer({size:32,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});function u(t,n,r){let i=f;i[0]=t[0],i[1]=t[1],i[2]=t[2],i[3]=0,i[4]=n[0],i[5]=n[1],i[6]=n[2],i[7]=r,e.queue.writeBuffer(l,0,i)}let d=e.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),f=new Float32Array(8),p=e.createBuffer({size:16,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),m=new Float32Array(4);function h(t,n){m[0]=t[0],m[1]=t[1],m[2]=t[2],m[3]=n,e.queue.writeBuffer(p,0,m)}function g(t){e.queue.writeBuffer(d,0,t)}function _(t,r,a){let o=e.createBuffer({size:Sn*4,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),s={model:t,modelBuf:o,bindGroup:e.createBindGroup({layout:i,entries:[{binding:0,resource:{buffer:d}},{binding:1,resource:c},{binding:2,resource:t.texture.createView()},{binding:3,resource:{buffer:l}},{binding:4,resource:{buffer:n.lightBuf}},{binding:5,resource:n.shadowSampler},{binding:6,resource:n.shadowView},{binding:7,resource:{buffer:o}},{binding:8,resource:{buffer:p}}]}),scratch:new Float32Array(Sn)};return v(s,r,a),s}function v(t,n,r){let i=t.scratch;i.set(n,0),i[16]=r?r[0]:1,i[17]=r?r[1]:1,i[18]=r?r[2]:1,i[19]=r?r[3]:0,e.queue.writeBuffer(t.modelBuf,0,i)}function y(e){if(e)try{e.modelBuf.destroy()}catch{}}function b(e){e.setPipeline(o)}function x(e){e.setPipeline(s)}function S(e,t){e.setBindGroup(0,t.bindGroup),e.setVertexBuffer(0,t.model.vao.posBuf),e.setVertexBuffer(1,t.model.vao.nrmBuf),e.setIndexBuffer(t.model.vao.idxBuf,t.model.vao.indexFormat),e.drawIndexed(t.model.vao.indexCount)}function ee(e,t){e.setBindGroup(0,t.bindGroup),e.setVertexBuffer(0,t.model.vao.posBuf),e.setVertexBuffer(1,t.model.vao.nrmBuf),e.setVertexBuffer(2,t.model.vao.uvBuf),e.setIndexBuffer(t.model.vao.idxBuf,t.model.vao.indexFormat),e.drawIndexed(t.model.vao.indexCount)}return{createInstance:_,updateInstance:v,destroyInstance:y,beginModels:b,draw:ee,beginOutlines:x,drawOutline:S,setOutlineStyle:h,setFog:u,setVP:g}}var Sn=20,Cn={food:`farm`,wood:`sawmill`,stone:`quarry`,gold:`gold-mine`,amber:`amber-vein`},wn={food:`Пашня`,wood:`Лесопилка`,stone:`Каменоломня`,gold:`Рудник`,amber:`Янтарная жила`};function Tn(e){return e>=25?5:e>=19?4:e>=13?3:e>=7?2:1}function En(){try{let e=window.parent;if(e&&e!==window){let t=e.mpWorldSnapshot;if(typeof t==`function`){let e=t();if(e)return e}if(e.W)return e.W}}catch{}return null}function Dn(){let e=En();return!e||!e.players[0]?null:{x:e.players[0].x,y:e.players[0].y}}var On=16;function kn(e,t,n){let r=En();if(!r)return null;let i=[],a=e!==void 0&&t!==void 0&&n!==void 0&&!!r.mapChunks,o=[];if(a){let i=Math.floor((e-n)/On),a=Math.floor((e+n)/On),s=Math.floor((t-n)/On),c=Math.floor((t+n)/On);for(let e=s;e<=c;e++)for(let t=i;t<=a;t++){let n=r.mapChunks[t+`,`+e];if(n)for(let e of n)o.push(e)}}else for(let e in r.map)o.push(e);let s=n===void 0?1/0:n*n,c=n!==void 0&&e!==void 0&&t!==void 0,l=new Map;for(let e of r.players)l.set(e.id,e);let u=r.players[0]?r.players[0].id:-1;for(let n of o){let a=r.map[n];if(a){if(c){let n=a.x-e,r=a.y-t;if(n*n+r*r>s)continue}if(a.t===`city`){let e=l.get(a.pid),t=e?e.race:`human`,r=e?Math.max(1,Math.min(5,Tn(e.b.hall))):1,o=!!e&&e.id===u,s=e?e.nick??`?`:`?`,c=e?`Ратуша `+e.b.hall:``;i.push({key:n,x:a.x+.5,y:a.y+.5,gx:a.x,gy:a.y,kind:0,model:`/models/castles/${t}-${r}.glb`,scale:10,own:o,nm:s,lv:c})}else if(a.t===`camp`||a.t===`fort`){let e=(a.t===`fort`?`Форт`:`Лагерь`)+` варваров`,t=`ур. `+(a.lv??`?`);i.push({key:n,x:a.x+.5,y:a.y+.5,gx:a.x,gy:a.y,kind:1,model:`/models/camps/barbarians.glb`,scale:a.t===`fort`?6.5:5,nm:e,lv:t})}else if(a.t===`node`){let e=Cn[a.res]||`farm`,t=wn[a.res]||`Точка`,r=`ур. `+(a.lv??`?`);i.push({key:n,x:a.x+.5,y:a.y+.5,gx:a.x,gy:a.y,kind:2,model:`/models/resources/${e}.glb`,scale:5,nm:t,lv:r})}}}return i}function An(e){let t=0;for(let n in e)for(let r in e[n])t+=e[n][+r]||0;return t}function jn(){try{let e=window.parent;if(e&&e!==window){let t=e.mpWorldSnapshot;if(typeof t==`function`){let e=t();if(e)return e}if(e.W)return e.W}}catch{}return null}var Mn=.004;function Nn(e,t){let n=e.path,r=e.pathCum;if(!n||n.length<2)return n&&n[0]||{x:e.tx,y:e.ty};let i=t*(e.pathLen??0);for(let e=1;e<r.length;e++)if(r[e]>=i){let t=r[e]-r[e-1],a=t>0?(i-r[e-1])/t:0,o=n[e-1],s=n[e];return{x:o.x+(s.x-o.x)*a,y:o.y+(s.y-o.y)*a}}return n[n.length-1]}function Pn(){let e=jn();if(!e||!e.marches)return null;let t=e.players[0]?e.players[0].id:-1,n=new Map;for(let t of e.players)n.set(t.id,t);let r=[];for(let i of e.marches){let a=i.state===`gather`||i.state===`siege`||i.state===`hold`?{x:i.tx,y:i.ty}:Nn(i,Math.max(0,Math.min(1,(e.t-i.t0)/Math.max(1,i.t1-i.t0)))),o=n.get(i.pid),s=NaN;if(i.state!==`gather`&&i.state!==`siege`&&i.state!==`hold`){let t=Math.max(0,Math.min(1,(e.t-i.t0)/Math.max(1,i.t1-i.t0))),n=Nn(i,Math.max(0,t-Mn)),r=Nn(i,Math.min(1,t+Mn)),a=r.x-n.x,o=r.y-n.y;a*a+o*o>1e-12&&(s=Math.atan2(a,o))}let c=i.state===`siege`&&i.data&&i.data.battle?i.data.battle:null,l=c?{round:c.round??0,revealFromRound:c.revealFromRound??0,retreating:!!(c.retreatRequested||c.retreated),attHpLeft:c.attHpLeft??0,attStartHp:c.attStartHp??1,revealFromAttHp:c.revealFromAttHp??c.attHpLeft??0,defHpLeft:c.defHpLeft??0,defStartHp:c.defStartHp??1,revealFromDefHp:c.revealFromDefHp??c.defHpLeft??0,revealStart:c.revealStart??0,revealAt:c.revealAt??0}:null;r.push({x:a.x,y:a.y,own:i.pid===t,id:i.id,nick:o?.nick??o?.name??`?`,unitsTotal:An(i.units),state:i.state,tx:i.tx,ty:i.ty,t1:i.t1,battle:l,race:o?.race??`human`,genId:o&&o.gen&&o.gen.id!=null?o.gen.id:null,hasGen:!!(i.hasGen??(i.data&&i.data.has_gen)),scout:i.mode===`scout`||i.mode===`scoutmarch`,yaw:s})}return r}var Fn=document.getElementById(`status`),$=(()=>{try{if(/[?&]debug=1\b/.test(location.search))return!0;if(window.parent&&window.parent!==window)return/[?&]debug=1\b/.test(window.parent.location.search)}catch{}return!1})();$&&(Fn.style.display=`block`);function In(e){$&&(Fn.textContent=e.join(`
`))}function Ln(e){Fn.style.display=`block`,Fn.textContent=e.join(`
`)}async function Rn(){let e=[];function t(t){Ln([...e,t]);try{window.parent&&window.parent!==window&&typeof window.parent.forceCityView==`function`&&window.parent.forceCityView()}catch{}}if(!(`gpu`in navigator)){t(`WebGPU: navigator.gpu отсутствует.`);return}await Ht(),e.push(`рельеф: настоящие данные высот загружены`);let n=document.getElementById(`hmVersion`);n&&(n.textContent=`h6`);let r={x:42,y:22},i=[.6,.52,.4],a=35e-5,o=Dn(),s=o??r,c=kn(s.x,s.y,192),l=c!==null;window.parent!==window&&!$&&(Fn.style.display=`none`);let d=c??[{key:`demo-0`,x:43,y:14,gx:43,gy:14,kind:0,model:`/models/castles/human-1.glb`,scale:10,nm:`Замок`,lv:`демо`},{key:`demo-1`,x:50,y:20,gx:50,gy:20,kind:1,model:`/models/camps/barbarians.glb`,scale:5,nm:`Лагерь`,lv:`демо`},{key:`demo-2`,x:55,y:12,gx:55,gy:12,kind:2,model:`/models/resources/farm.glb`,scale:5,nm:`Пашня`,lv:`демо`},{key:`demo-3`,x:30,y:30,gx:30,gy:30,kind:2,model:`/models/resources/quarry.glb`,scale:5,nm:`Каменоломня`,lv:`демо`}];e.push(l?`данные: настоящая партия, сущностей — ${d.length}`:`данные: демо (window.parent.W недоступен)`);let f=u(),p={x:[],y:[]},m={value:[]},h=new Map,g=new Map,_=new Map,v=new Map,y=new Map,b=new Map,x=new Map,S=new Map;function ee(e){let t=Fe(f);return Me(f,t,p),Me(f,t,m),p.x[t]=e.x,p.y[t]=e.y,m.value[t]=e.kind,h.set(t,e.model),g.set(t,e.scale),v.set(t,e.nm),y.set(t,e.lv),b.set(t,!!e.own),x.set(t,{x:e.gx,y:e.gy}),S.set(e.key,t),Xt(e.x,e.y,e.scale*1.4),t}for(let e of d)ee(e);let C=Array.from(Ee(f,[p,m]));e.push(`bitECS: сущностей — ${C.length}`);let w=await navigator.gpu.requestAdapter();if(!w){t(`WebGPU: адаптер не найден.`);return}let te=await w.requestDevice();function ne(e){let t=document.getElementById(`gpu-error-banner`);t||(t=document.createElement(`div`),t.id=`gpu-error-banner`,t.style.cssText=`position:fixed;left:0;right:0;bottom:0;z-index:99999;background:#4a0f0f;color:#fff;font:11px/1.4 monospace;padding:6px 8px;max-height:40vh;overflow:auto;white-space:pre-wrap;`,document.body.appendChild(t)),t.textContent+=(t.textContent?`
---
`:``)+e}te.addEventListener(`uncapturederror`,e=>{let t=e.error.message;console.error(`WebGPU error:`,t),ne(t)});let re=`fb-gpu-reload-at`,ie=Number(sessionStorage.getItem(re)||0),ae=ie&&Date.now()-ie<6e4;te.lost.then(e=>{if(console.error(`WebGPU device lost:`,e.reason,e.message),e.reason!==`destroyed`){if(ae){ne(`WebGPU-устройство теряется повторно (${e.reason}) — похоже, объёмная карта нестабильна на этом устройстве/браузере. Карта города ниже работает независимо от WebGPU.`);try{window.parent&&window.parent!==window&&typeof window.parent.forceCityView==`function`&&window.parent.forceCityView()}catch{}return}ne(`WebGPU-устройство потеряно (${e.reason}): ${e.message}\nПерезагрузка через 2с...`),sessionStorage.setItem(re,String(Date.now())),setTimeout(()=>location.reload(),2e3)}});let T=document.getElementById(`gpu`),oe=T.getContext(`webgpu`);if(!oe){t(`WebGPU: getContext('webgpu') вернул null.`);return}let se=navigator.gpu.getPreferredCanvasFormat();function ce(){let e=T.clientWidth,t=T.clientHeight;if(e<=0||t<=0)return;let n=Math.min(2,window.devicePixelRatio||1),r=Math.max(1,Math.floor(e*n)),i=Math.max(1,Math.floor(t*n));T.width!==r&&(T.width=r),T.height!==i&&(T.height=i)}let le=()=>T.offsetParent===null&&T.clientWidth===0;ce(),new ResizeObserver(ce).observe(T),oe.configure({device:te,format:se,alphaMode:`opaque`}),e.push(`WebGPU: устройство получено, формат — ${se}`);let E=await Mt(te,oe,se);function ue(e,t){return e+`,`+t}function de(e,t){try{let n=window.parent;n&&n!==window&&typeof n.ensureWorldChunk==`function`&&n.ensureWorldChunk(e,t)}catch{}}let fe=new Map;function pe(e,t){return[e[0]*t,e[1]*t,e[2]*t]}function me(e,t,n){if(Zt(e,t))return!0;for(let r=0;r<8;r++){let i=r/8*Math.PI*2;if(Zt(e+Math.cos(i)*n,t+Math.sin(i)*n))return!0}return!1}function D(e,t,n,r){for(let i of C){let a=p.x[i]-e,o=p.y[i]-t,s=(g.get(i)??5)*n+r;if(a*a+o*o<s*s)return!0}return!1}function he(e,t){return e>1.36?t<.62?`spruce`:t<.94?`pine`:`dead`:t<.58?`broadleaf`:t<.8?`birch`:t<.94?`spruce`:`dead`}function O(e,t){let n=[];for(let r=0;r<4;r++)for(let i=0;i<4;i++){let a=e*4+i,o=t*4+r;if(Y(a,o,13122)>=.65)continue;let s=.175+Y(a,o,J+778)*.65,c=.175+Y(a,o,J+779)*.65,l=e*16+i*4+s*4,u=t*16+r*4+c*4,d=l+(Rt(l/8.5,u/8.5,J+790)*2-1)*2,f=u+(Rt(l/8.5,u/8.5,J+791)*2-1)*2;if(me(d,f,1.5)||D(d,f,.54,.68))continue;let p=Y(a,o,J+781)*Math.PI*2,m=.85+Y(a,o,J+782)*.3,h=Z(d,f),g=h*13,_=.0315+.5355*$t(d,f),v=Y(a,o,J+780)<_,y=1+Y(a,o,J+785)*1.3,b=.8+Y(a,o,J+786)*.5;if(v){let e=he(h,Y(a,o,J+780));e===`broadleaf`&&Y(a,o,13132)<.35&&(e=`autumn`);let t=e===`spruce`||e===`pine`?Je:Ye,r=t[Math.floor(Y(a,o,J+784)*t.length)];n.push({x:d,y:g,z:f,scale:[b,y,b],yaw:p,color:pe(r,m),kind:e})}else{let e=.019+.056999999999999995*Math.min(1,h/1.6);if(Y(a,o,13140)>=e)continue;let t=Qe[Math.floor(Y(a,o,J+784)*Qe.length)],r=.6+Y(a,o,J+785)*.9,i=.6+Y(a,o,J+786)*.9;n.push({x:d,y:g,z:f,scale:[i,r,i],yaw:p,color:pe(t,m),kind:`rock`})}}for(let r=0;r<8;r++)for(let i=0;i<8;i++){let a=e*8+i,o=t*8+r;if(Y(a,o,13232)>=.14875)continue;let s=Y(a,o,J+888),c=Y(a,o,J+889),l=e*16+i*2+s*2,u=t*16+r*2+c*2;if(me(l,u,.4)||D(l,u,.36,.17))continue;let d=Z(l,u);if(d>2)continue;let f=d*13,p=Y(a,o,J+890)*Math.PI*2,m=.8+Y(a,o,J+891)*.4,h=Xe[Math.floor(Y(a,o,J+892)*Xe.length)],g=.8+Y(a,o,J+893)*.6;n.push({x:l,y:f,z:u,scale:[g,g,g],yaw:p,color:pe(h,m),kind:`grass`})}let r=16/3;for(let i=0;i<r;i++)for(let a=0;a<r;a++){let o=e*r+a,s=t*r+i;if(Y(o,s,13342)>=.07875)continue;let c=Y(o,s,J+998),l=Y(o,s,J+999),u=e*16+a*3+c*3,d=t*16+i*3+l*3;if(me(u,d,.9)||D(u,d,.44,.34))continue;let f=Z(u,d);if(f>2)continue;let p=f*13,m=Y(o,s,J+1e3)*Math.PI*2,h=.85+Y(o,s,J+1001)*.3,g=Ze[Math.floor(Y(o,s,J+1002)*Ze.length)],_=.9+Y(o,s,J+1003)*.7;n.push({x:u,y:p,z:d,scale:[_,_,_],yaw:m,color:pe(g,h),kind:`bush`})}return n}function ge(){let e=[];for(let t of fe.values())e.push(...t);E.setDecor(e),window.__decorCount=e.length,window.__decorList=e}let _e=.5,ve=1,ye=(e,t)=>e*.8+t*.2,k=new Set,be=new Set,A=[],xe=null,Se=null;function j(e,t,n=!1){let r=Math.floor(e/16),i=Math.floor(t/16);if(!n&&r===xe&&i===Se)return;xe=r,Se=i;let a=!1;for(let e=-3;e<=3;e++)for(let t=-3;t<=3;t++){let n=r+t,o=i+e,s=ue(n,o);k.has(s)||be.has(s)||(be.add(s),A.push({cx:n,cz:o,key:s}),a=!0)}let o=!1;for(let e of Array.from(k)){let[t,n]=e.split(`,`).map(Number);Math.max(Math.abs(t-r),Math.abs(n-i))>5&&(E.removeTerrainChunk(e),k.delete(e),fe.delete(e),o=!0)}for(let e of Array.from(be)){let[t,n]=e.split(`,`).map(Number);Math.max(Math.abs(t-r),Math.abs(n-i))>5&&(be.delete(e),a=!0)}a&&(A=A.filter(e=>be.has(e.key)),A.sort((e,t)=>(e.cx-r)**2+(e.cz-i)**2-((t.cx-r)**2+(t.cz-i)**2))),window.__terrainChunkCount=k.size,o&&ge()}function Ce(e){let t=!1,n=0;for(;A.length&&!(n>0&&performance.now()+_e>e);){let{cx:e,cz:r,key:i}=A.shift();if(!be.has(i))continue;be.delete(i);let a=performance.now(),o=e*16,s=r*16,c=ln(o,s,o+16,s+16,1);E.setTerrainChunk(i,c),k.add(i),de(e,r),fe.set(i,O(e,r)),t=!0,_e=ye(_e,performance.now()-a),n++}return t&&(window.__terrainChunkCount=k.size,ge()),n}function we(e,t){let n=ue(e,t);if(!k.has(n))return;let r=e*16,i=t*16;E.setTerrainChunk(n,ln(r,i,r+16,i+16,1))}function Te(e,t,n,r){let i=Math.floor(n/16),a=Math.floor(r/16),o=(i-3)*16,s=(i+3+1)*16,c=(a-3)*16,l=(a+3+1)*16,u=e*64,d=t*64;return u>=o&&u+64<=s&&d>=c&&d+64<=l}let M=new Set,N=new Set,P=[],De=null,Oe=null;function ke(e,t,n=!1){let r=Math.floor(e/64),i=Math.floor(t/64);if(!n&&r===De&&i===Oe)return;De=r,Oe=i;let a=!1;for(let n=-2;n<=2;n++)for(let o=-2;o<=2;o++){let s=r+o,c=i+n,l=`far:`+s+`,`+c;M.has(l)||N.has(l)||Te(s,c,e,t)||(N.add(l),P.push({cx:s,cz:c,rkey:l}),a=!0)}for(let n of Array.from(M)){let[a,o]=n.slice(4).split(`,`).map(Number),s=Math.max(Math.abs(a-r),Math.abs(o-i))>3,c=Te(a,o,e,t);(s||c)&&(E.removeTerrainChunk(n),M.delete(n))}for(let e of Array.from(N)){let[t,n]=e.slice(4).split(`,`).map(Number);Math.max(Math.abs(t-r),Math.abs(n-i))>3&&(N.delete(e),a=!0)}a&&(P=P.filter(e=>N.has(e.rkey)),P.sort((e,t)=>(e.cx-r)**2+(e.cz-i)**2-((t.cx-r)**2+(t.cz-i)**2))),window.__farChunkCount=M.size}function Ae(e,t){let n=0;for(;P.length&&!(!(n===0&&t)&&performance.now()+ve>e);){let{cx:e,cz:t,rkey:r}=P.shift();if(!N.has(r))continue;N.delete(r);let i=performance.now(),a=e*64,o=t*64,s=ln(a,o,a+64,o+64,4,.35);E.setTerrainChunk(r,s),M.add(r),ve=ye(ve,performance.now()-i),n++}window.__farChunkCount=M.size}let F=xn(te,se,E.getShadowResources()),I=new Map;function L(e){let t=I.get(e);return t||(t=vn(e).then(e=>bn(te,e)),I.set(e,t)),t}let je=new Set(Array.from(C,e=>h.get(e)));await Promise.allSettled(Array.from(je,e=>L(e)));let R=new Map,Ne=0,z=0;for(let t of C){let n=p.x[t],r=p.y[t],i=Z(n,r)*13;_.set(t,i);let a=Ue(n,i,r,0,g.get(t)??5),o=h.get(t);try{let e=await L(o);R.set(t,F.createInstance(e,a)),Ne++}catch(t){z++,e.push(`модель: ошибка на ${o} — ${t instanceof Error?t.message:String(t)}`)}}e.push(`модели: загружено ${Ne}/${C.length}${z?`, ошибок: `+z:``}`),In(e),window.__ecsFound=C.length,window.__foundPositions=()=>C.map(e=>({x:p.x[e],z:p.y[e],scale:g.get(e)??5}));let Pe=o?o.x:r.x,Le=o?o.y:r.y,B={yaw:0,pitch:.55,dist:42,target:[Pe,Z(Pe,Le)*13+2,Le]},V=hn(T,B);j(B.target[0],B.target[2],!0),ke(B.target[0],B.target[2],!0);let He=performance.now()+40;Ce(He),Ae(He,!0);let $e=ln(-It,-600,It,600,12,1.2);E.setTerrainChunk(`world-backdrop`,$e),e.push(`рельеф: чанков ${k.size} (16×16) + дальних ${M.size} (64×64, шаг 4) + задник (шаг 12, весь мир), в очереди ещё ${A.length+P.length}`),In(e),window.__coverageCheck=(e,t)=>{for(let n of k){let[r,i]=n.split(`,`).map(Number),a=r*16,o=i*16;if(e>=a&&e<a+16&&t>=o&&t<o+16)return`near`}for(let n of M){let[r,i]=n.slice(4).split(`,`).map(Number),a=r*64,o=i*64;if(e>=a&&e<a+64&&t>=o&&t<o+64)return`far`}return null},Object.defineProperty(window,"cam",{value:{get tx(){return B.target[0]},set tx(e){B.target[0]=e,V.stopAuto()},get ty(){return B.target[1]},set ty(e){B.target[1]=e,V.stopAuto()},get tz(){return B.target[2]},set tz(e){B.target[2]=e,V.stopAuto()},get dist(){return B.dist},set dist(e){B.dist=e,V.stopAuto()},get pitch(){return B.pitch},set pitch(e){B.pitch=e,V.stopAuto()}}}),window.H=(e,t)=>Z(e,t)*13,window.__camState=()=>({yaw:B.yaw,pitch:B.pitch,dist:B.dist,target:[...B.target]}),window.__isAutoOrbiting=()=>V.isAutoOrbiting();let et=document.getElementById(`coordX`),U=document.getElementById(`coordY`),tt=document.getElementById(`coordGo`),W=!1;for(let e of[et,U])e.addEventListener(`input`,()=>{W=!0});function nt(){let e=parseFloat(et.value),t=parseFloat(U.value);!isFinite(e)||!isFinite(t)||(B.target[0]=Math.max(-It,Math.min(It,e)),B.target[2]=Math.max(-600,Math.min(600,t)),B.target[1]=Z(B.target[0],B.target[2])*13+2,V.stopAuto(),W=!1)}tt.addEventListener(`click`,nt);for(let e of[et,U])e.addEventListener(`keydown`,t=>{t.key===`Enter`&&(t.preventDefault(),nt(),e.blur())});let rt=new Float32Array(16),G=[0,0,0],it=document.getElementById(`selected`),at=3.2,ot=2.6,st=e=>e.scout?ot:at,ct=e=>Z(e.x,e.y)*13+st(e),lt=e=>Z(e.x,e.y)*13+st(e)*.5,ut=[1,.86,.42],dt=1500,ft=null,pt=null,mt=[.35,.85,.45],K=null,ht=null,gt=null;window.startFollowMarch=e=>{V.stopAuto(),gt=e},V.onInteract(()=>{gt=null});function _t(e){ht=null,K=e;let t=(v.get(e)??`?`)+` · `+(y.get(e)??`?`);window.__markerActive=!0,window.__selectedLabel=t,it.textContent=t,it.style.display=`block`}function vt(){K=null,window.__markerActive=!1,window.__selectedLabel=null,it.style.display=`none`}function yt(e,t){let n=T.width/Math.max(1,T.height),r=Math.tan(bt/2),i=e/T.width*2-1,a=1-t/T.height*2,o=H(Re(G,B.target)),s=H(ze([0,1,0],o)),c=ze(o,s),l=H([i*n*r*s[0]+a*r*c[0]-o[0],i*n*r*s[1]+a*r*c[1]-o[1],i*n*r*s[2]+a*r*c[2]-o[2]]);return{origin:G,dir:l}}let bt=.72;function xt(e,t){let n=0;for(let r=2;r<=400;r+=2){let i=e[0]+t[0]*r;if(e[1]+t[1]*r-Z(i,e[2]+t[2]*r)*13<=0){let i=n,a=r;for(let n=0;n<12;n++){let n=(i+a)/2,r=e[0]+t[0]*n,o=e[2]+t[2]*n;e[1]+t[1]*n-Z(r,o)*13>0?i=n:a=n}return{t:a,x:e[0]+t[0]*a,z:e[2]+t[2]*a}}n=r}return null}function St(e,t){try{let n=window.parent;n&&n!==window&&typeof n.renderCartoucheFor==`function`&&n.renderCartoucheFor(e,t)}catch{}}function Ct(e){try{let t=window.parent;t&&t!==window&&typeof t.renderMarchCartoucheFor==`function`&&t.renderMarchCartoucheFor(e)}catch{}}function wt(e){let t=Ke(rt,e);return t.w<=.001?null:{sx:(t.x/t.w*.5+.5)*T.width,sy:(1-(t.y/t.w*.5+.5))*T.height,w:t.w}}function Tt(e,t){let n=.5*T.height/Math.tan(bt/2),r=null,i=1/0;for(let a of C){let o=p.x[a],s=p.y[a],c=g.get(a)??5,l=[o,(_.get(a)??Z(o,s)*13)+c*.5,s],u=wt(l);if(!u)continue;let d=Math.max(26,n*c/u.w),f=e-u.sx,m=t-u.sy,h=f*f+m*m;h>d*d||h>=i||(i=h,r={kind:`entity`,eid:a,distToCam:Math.hypot(o-G[0],l[1]-G[1],s-G[2])})}for(let a of q){let o=[a.x,lt(a),a.y],s=wt(o);if(!s)continue;let c=Math.max(3,st(a)*.5),l=Math.max(26,n*c/s.w),u=e-s.sx,d=t-s.sy,f=u*u+d*d;f>l*l||f>=i||(i=f,r={kind:`march`,march:a,distToCam:Math.hypot(a.x-G[0],o[1]-G[1],a.y-G[2])})}let{origin:a,dir:o}=yt(e,t),s=xt(a,o);return r&&!(s!==null&&s.t+5<r.distToCam)?r.kind===`entity`?{kind:`entity`,eid:r.eid,t:r.distToCam}:{kind:`march`,march:r.march,t:r.distToCam}:s===null?null:{kind:`ground`,x:s.x,z:s.z,t:s.t}}V.onTap((e,t)=>{let n=T.getBoundingClientRect(),r=Tt((e-n.left)*(T.width/n.width),(t-n.top)*(T.height/n.height));if(r?.kind===`entity`){_t(r.eid);let e=x.get(r.eid);e&&St(e.x,e.y);return}if(r?.kind===`march`){vt(),ht=r.march.id,window.__selectedMarchId=r.march.id,Ct(r.march.id);return}vt(),ht=null,r?.kind===`ground`&&St(Math.floor(r.x),Math.floor(r.z))});function Et(e,t){let n=T.getBoundingClientRect();return[(e-n.left)*(T.width/n.width),(t-n.top)*(T.height/n.height)]}function Dt(e,t){let[n,r]=Et(e,t),i=Tt(n,r);if(!i)return null;if(i.kind===`ground`)return{x:i.x,z:i.z};if(i.kind===`entity`){let e=x.get(i.eid);if(e)return{x:e.x+.5,z:e.y+.5}}return i.kind===`march`?{x:i.march.x,z:i.march.y}:null}function Ot(e,t){let n=Dt(e,t);if(!n){pt=null;return}pt={x:n.x,y:Z(n.x,n.z)*13+3,z:n.z,color:mt}}V.onGrab((e,t)=>{let[n,r]=Et(e,t),i=Tt(n,r);return i?.kind!==`march`||!i.march.own?!1:(ft=i.march.id,Ot(e,t),!0)},(e,t)=>{ft!==null&&Ot(e,t)},(e,t)=>{let n=ft;if(ft=null,pt=null,n===null||!isFinite(e)||!isFinite(t))return;let r=Dt(e,t);if(r)try{let e=window.parent;e&&e!==window&&typeof e.redirectMarchTo==`function`&&e.redirectMarchTo(n,Math.floor(r.x),Math.floor(r.z))}catch{}});let kt=!1,At=0;async function jt(){let e=kn(B.target[0],B.target[2],192);if(!e)return;let t=new Set,n=[],r=new Set;for(let i of e){t.add(i.key);let e=S.get(i.key);if(e!==void 0){if(v.set(e,i.nm),y.set(e,i.lv),b.set(e,!!i.own),K===e&&_t(e),h.get(e)!==i.model){h.set(e,i.model),g.set(e,i.scale);let t=p.x[e],r=p.y[e],a=Z(t,r)*13;_.set(e,a);let o=Ue(t,a,r,0,i.scale);n.push(L(i.model).then(t=>{F.destroyInstance(R.get(e)),R.set(e,F.createInstance(t,o))}).catch(()=>{}))}continue}let a=ee(i),o=Z(i.x,i.y)*13;_.set(a,o);let s=Ue(i.x,o,i.y,0,i.scale);n.push(L(i.model).then(e=>{F.destroyInstance(R.get(a)),R.set(a,F.createInstance(e,s))}).catch(()=>{})),r.add(ue(Math.floor(i.x/16),Math.floor(i.y/16))),we(Math.floor(i.x/16),Math.floor(i.y/16))}for(let[e,n]of Array.from(S))t.has(e)||(r.add(ue(Math.floor(p.x[n]/16),Math.floor(p.y[n]/16))),Ie(f,n),F.destroyInstance(R.get(n)),R.delete(n),h.delete(n),g.delete(n),_.delete(n),v.delete(n),y.delete(n),b.delete(n),x.delete(n),S.delete(e),K===n&&vt());await Promise.allSettled(n),C=Array.from(Ee(f,[p,m]));let i=!1;for(let e of r){if(!k.has(e))continue;let[t,n]=e.split(`,`).map(Number);fe.set(e,O(t,n)),i=!0}i&&ge(),At++,window.__ecsFound=C.length,window.__syncCount=At}l&&setInterval(()=>{le()||kt||(kt=!0,jt().catch(e=>console.error(`live sync:`,e)).finally(()=>{kt=!1}))},3e3);let q=[];function Nt(){if(!l){q=[];return}q=Pn()||[],window.__marchPositions=q}let Pt=new Set([`human`,`dwarf`,`elf`,`undead`]),Ft=[.62,1.14,.72,.32],Lt=[1.22,.55,.5,.32];function zt(e){let t=Pt.has(e.race)?e.race:`human`;return e.scout?`/models/marches/scout-${t}.glb`:e.hasGen?`/models/marches/gen-${t}-${+(e.genId===1)}.glb`:`/models/marches/army-${t}.glb`}let X=new Map;function Bt(){for(let e of q){let t=zt(e),n=X.get(e.id);if(n&&n.path!==t&&(F.destroyInstance(n.inst??void 0),X.delete(e.id),n=void 0),!n){n={path:t,inst:null,yaw:Number.isFinite(e.yaw)?e.yaw:0},X.set(e.id,n);let r=n,i=e.id;L(t).then(e=>{X.get(i)===r&&(r.inst=F.createInstance(e,Ue(0,-1e6,0,0,1),Ft))},()=>{})}if(Number.isFinite(e.yaw)&&(n.yaw=e.yaw),n.inst){let t=e.scout?ot:at;F.updateInstance(n.inst,Ue(e.x,Z(e.x,e.y)*13,e.y,n.yaw,t),e.own?Ft:Lt)}}if(X.size>q.length){let e=new Set(q.map(e=>e.id));for(let[t,n]of X)e.has(t)||(F.destroyInstance(n.inst??void 0),X.delete(t))}}let Vt=document.getElementById(`labels`),Ut=new Map,Wt=We(),Gt=1024;function Kt(){let e=new Set,t=T.clientWidth,n=T.clientHeight;for(let r of C){let i=p.x[r],a=p.y[r],o=i-B.target[0],s=a-B.target[2];if(o*o+s*s>Gt)continue;let c=(_.get(r)??Z(i,a)*13)+(g.get(r)??5)*.6+1.1,l=Ge(rt,i,c,a,Wt);if(l.w<=.001)continue;let u=(l.x/l.w*.5+.5)*t,d=(1-(l.y/l.w*.5+.5))*n;if(u<-40||u>t+40||d<-40||d>n+40)continue;e.add(r);let f=Ut.get(r);if(!f){let e=document.createElement(`div`);e.className=`wlabel`;let t=document.createElement(`div`);t.className=`nm`;let n=document.createElement(`div`);n.className=`lv`,e.appendChild(t),e.appendChild(n),Vt.appendChild(e),f={root:e,nm:t,lv:n,lastNm:``,lastLv:``,lastMine:!1},Ut.set(r,f)}let m=v.get(r)??`?`;f.lastNm!==m&&(f.nm.textContent=m,f.lastNm=m);let h=!!b.get(r);f.lastMine!==h&&(f.nm.classList.toggle(`mine`,h),f.lastMine=h);let x=y.get(r)??``;f.lastLv!==x&&(f.lv.textContent=x,f.lastLv=x),f.root.style.transform=`translate(${u.toFixed(1)}px,${d.toFixed(1)}px) translate(-50%,-100%)`}for(let[t,n]of Ut)e.has(t)||(n.root.remove(),Ut.delete(t))}let qt=new Map;function Jt(e,t,n,r){if(!r||!n||r<=n)return t;let i=Math.max(0,Math.min(1,(Date.now()-n)/(r-n)));return e+(t-e)*i}function Yt(){let e=new Set,t=T.clientWidth,n=T.clientHeight;for(let r of q){let i=r.battle;if(!i)continue;let a=r.x-B.target[0],o=r.y-B.target[2];if(a*a+o*o>Gt)continue;let s=ct(r)+1.6,c=Ke(rt,[r.x,s,r.y]);if(c.w<=.001)continue;let l=(c.x/c.w*.5+.5)*t,u=(1-(c.y/c.w*.5+.5))*n;if(l<-60||l>t+60||u<-60||u>n+60)continue;e.add(r.id);let d=qt.get(r.id);if(!d){let e=document.createElement(`div`);e.className=`blabel`;let t=document.createElement(`div`);t.className=`btitle`;let n=document.createElement(`div`);n.className=`bbar atk`;let i=document.createElement(`i`);n.appendChild(i);let a=document.createElement(`div`);a.className=`bbar def`;let o=document.createElement(`i`);a.appendChild(o),e.appendChild(t),e.appendChild(n),e.appendChild(a),Vt.appendChild(e),d={root:e,title:t,atkFill:i,defFill:o},qt.set(r.id,d)}let f=i.retreating,p=!f&&i.revealFromRound===0;d.root.className=`blabel`+(f?` retreat`:p?` deploy`:``),d.title.textContent=f?`Отступление`:p?`Развёртывание`:`Бой — раунд `+i.round;let m=Math.max(0,Math.min(100,Jt(i.revealFromAttHp,i.attHpLeft,i.revealStart,i.revealAt)/Math.max(1,i.attStartHp)*100)),h=Math.max(0,Math.min(100,Jt(i.revealFromDefHp,i.defHpLeft,i.revealStart,i.revealAt)/Math.max(1,i.defStartHp)*100));d.atkFill.style.width=m.toFixed(1)+`%`,d.defFill.style.width=h.toFixed(1)+`%`,d.root.style.transform=`translate(${l.toFixed(1)}px,${u.toFixed(1)}px) translate(-50%,-100%)`}for(let[t,n]of qt)e.has(t)||(n.root.remove(),qt.delete(t))}let Qt=We();function en(e,t,n,r,i){let a=p.x[e],o=p.y[e],s=g.get(e)??5,c=(_.get(e)??0)+s*.6,l=a-G[0],u=c-G[1],d=o-G[2],f=l*l+u*u+d*d;if(f<3600)return!0;if(f>16900)return!1;let m=Ge(t,a,c,o,Qt);if(m.w<=.001)return!1;let h=(m.x/m.w*.5+.5)*n,v=(1-(m.y/m.w*.5+.5))*r,y=i*s/m.w+24;return h>-y&&h<n+y&&v>-y&&v<r+y}let Q={frame:0,chunks:0,render:0,labels:0,maxFrame:0,maxChunks:0,maxRender:0,maxLabels:0,n:0,worstFrame:0,worstChunks:0,worstRender:0,worstLabels:0};function tn(){Q.maxFrame=Math.max(Q.maxFrame,Q.frame),Q.maxChunks=Math.max(Q.maxChunks,Q.chunks),Q.maxRender=Math.max(Q.maxRender,Q.render),Q.maxLabels=Math.max(Q.maxLabels,Q.labels),++Q.n>=60&&(Q.worstFrame=Q.maxFrame,Q.worstChunks=Q.maxChunks,Q.worstRender=Q.maxRender,Q.worstLabels=Q.maxLabels,Q.maxFrame=Q.maxChunks=Q.maxRender=Q.maxLabels=0,Q.n=0,In([`кадр (худший из 60): ${Q.worstFrame.toFixed(1)} мс`,`  стройка чанков: ${Q.worstChunks.toFixed(1)} мс`,`  отрисовка:      ${Q.worstRender.toFixed(1)} мс`,`  подписи:        ${Q.worstLabels.toFixed(1)} мс`,`чанков ${window.__terrainChunkCount??0} · моделей в кадре ${window.__modelDrawCount??0} · сущностей ${window.__ecsFound??C.length} · декора ${window.__decorCount??0}`,`в очереди на стройку: ${A.length} ближних`])),window.__perf=Q}let nn=!1;function rn(e){let t=$?performance.now():0;try{an(e)}catch(e){console.error(`draw:`,e),nn||(nn=!0,ne(`Сбой в кадре: ${e instanceof Error?e.message:String(e)}`))}$&&(Q.frame=performance.now()-t,tn()),requestAnimationFrame(rn)}function an(e){if(le())return;V.isAutoOrbiting()&&(B.yaw=e*15e-5),V.update(e),Nt();let t=[];if(gt!==null){let e=q.find(e=>e.id===gt);e?(B.target[0]=e.x,B.target[2]=e.y,B.target[1]=Z(e.x,e.y)*13+1):gt=null}W||(et.value=B.target[0].toFixed(1),U.value=B.target[2].toFixed(1)),j(B.target[0],B.target[2]),ke(B.target[0],B.target[2]);let n=performance.now(),r=n+3;Ae(r,Ce(r)===0),$&&(Q.chunks=performance.now()-n);let o=[B.target[0]+Math.sin(B.yaw)*Math.cos(B.pitch)*B.dist,B.target[1]+Math.sin(B.pitch)*B.dist,B.target[2]+Math.cos(B.yaw)*Math.cos(B.pitch)*B.dist],s=Z(o[0],o[2])*13+2;o[1]<s&&(o[1]=s);let c=T.width/Math.max(1,T.height),l=Be(Ve(bt,c,.5,392),qe(o,B.target,[0,1,0]));rt=l,G=o,E.setVP(l),E.setFog(o,i,a,e/1e3),E.setSunTarget(B.target[0],B.target[2]);{let t=H(Re(o,B.target)),n=H(ze([0,1,0],t)),r=ze(t,n);E.setSkyCamera(n,r,t,Math.tan(bt/2),c,e/1e3)}F.setFog(o,i,a),F.setVP(l),ht!==null&&!q.some(e=>e.id===ht)&&(ht=null),pt&&t.push(pt),E.setMarkers(t),Bt(),window.__marchCount=q.length;let u=T.clientWidth,d=T.clientHeight,f=.5*d/Math.tan(bt/2),p=0,m=$?performance.now():0,h=K===null?ht===null?null:X.get(ht)?.inst??null:R.get(K)??null;if(h){let t=.5-.5*Math.cos(e%dt/dt*Math.PI*2);F.setOutlineStyle(ut,.003+.0017999999999999995*t)}E.frame({r:i[0],g:i[1],b:i[2],a:1},e=>{F.beginModels(e);for(let t of C){if(!en(t,l,u,d,f))continue;let n=R.get(t);n&&(F.draw(e,n),p++)}for(let t of X.values())t.inst&&(F.draw(e,t.inst),p++);h&&(F.beginOutlines(e),F.drawOutline(e,h))}),window.__modelDrawCount=p,$&&(Q.render=performance.now()-m);let g=$?performance.now():0;Kt(),Yt(),$&&(Q.labels=performance.now()-g)}requestAnimationFrame(rn),window.__engineReady=!0}Rn().catch(e=>{Ln([`Ошибка: ${e instanceof Error?e.message:String(e)}`]),console.error(e)});