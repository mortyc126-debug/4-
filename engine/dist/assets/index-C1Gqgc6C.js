...
`)}async function kn(){let e=[],t={x:42,y:22},n=[.6,.52,.4],r=.0042,i=vn(),a=i??t,o=bn(a.x,a.y,384),s=o!==null;window.parent!==window&&(Tn.style.display=`none`);let c=o??[{key:`demo-0`,x:43,y:14,gx:43,gy:14,kind:0,model:`/models/castles/human-1.glb`,scale:10,nm:`Замок`,lv:`демо`},{key:`demo-1`,x:50,y:20,gx:50,gy:20,kind:1,model:`/models/camps/barbarians.glb`,scale:5,nm:`Лагерь`,lv:`демо`},{key:`demo-2`,x:55,y:12,gx:55,gy:12,kind:2,model:`/models/resources/farm.glb`,scale:5,nm:`Пашня`,lv:`демо`},{key:`demo-3`,x:30,y:30,gx:30,gy:30,kind:2,model:`/models/resources/quarry.glb`,scale:5,nm:`Каменоломня`,lv:`демо`}];e.push(s?`данные: настоящая партия, сущностей — ${c.length}`:`данные: демо (window.parent.W недоступен)`);let l=u(),d={x:[],y:[]},f={value:[]},p=new Map,m=new Map,h=new Map,g=new Map,_=new Map,v=new Map,y=new Map;function b(e){let t=je(l);return ke(l,t,d),ke(l,t,f),d.x[t]=e.x,d.y[t]=e.y,f.value[t]=e.kind,p.set(t,e.model),m.set(t,e.scale),h.set(t,e.nm),g.set(t,e.lv),_.set(t,!!e.own),v.set(t,{x:e.gx,y:e.gy}),y.set(e.key,t),Ut(e.x,e.y,e.scale*1.4),t}for(let e of c)b(e);let x=Array.from(Ce(l,[d,f]));e.push(`bitECS: сущностей — ${x.length}`);function S(t){On([...e,t]);try{window.parent&&window.parent!==window&&typeof window.parent.forceCityView==`function`&&window.parent.forceCityView()}catch{}}if(!(`gpu`in navigator)){S(`WebGPU: navigator.gpu отсутствует.`);return}let ee=await navigator.gpu.requestAdapter();if(!ee){S(`WebGPU: адаптер не найден.`);return}let te=await ee.requestDevice();function C(e){let t=document.getElementById(`gpu-error-banner`);t||(t=document.createElement(`div`),t.id=`gpu-error-banner`,t.style.cssText=`position:fixed;left:0;right:0;bottom:0;z-index:99999;background:#4a0f0f;color:#fff;font:11px/1.4 monospace;padding:6px 8px;max-height:40vh;overflow:auto;white-space:pre-wrap;`,document.body.appendChild(t)),t.textContent+=(t.textContent?`
---
`:``)+e}te.addEventListener(`uncapturederror`,e=>{let t=e.error.message;console.error(`WebGPU error:`,t),C(t)});let ne=`fb-gpu-reload-at`,re=Number(sessionStorage.getItem(ne)||0),ie=re&&Date.now()-re<6e4;te.lost.then(e=>{if(console.error(`WebGPU device lost:`,e.reason,e.message),e.reason!==`destroyed`){if(ie){C(`WebGPU-устройство теряется повторно (${e.reason}) — похоже, объёмная карта нестабильна на этом устройстве/браузере. Карта города ниже работает независимо от WebGPU.`);try{window.parent&&window.parent!==window&&typeof window.parent.forceCityView==`function`&&window.parent.forceCityView()}catch{}return}C(`WebGPU-устройство потеряно (${e.reason}): ${e.message}\nПерезагрузка через 2с...`),sessionStorage.setItem(ne,String(Date.now())),setTimeout(()=>location.reload(),2e3)}});let w=document.getElementById(`gpu`),ae=w.getContext(`webgpu`);if(!ae){S(`WebGPU: getContext('webgpu') вернул null.`);return}let T=navigator.gpu.getPreferredCanvasFormat();function oe(){let e=w.clientWidth,t=w.clientHeight;if(e<=0||t<=0)return;let n=Math.min(2,window.devicePixelRatio||1),r=Math.max(1,Math.floor(e*n)),i=Math.max(1,Math.floor(t*n));w.width!==r&&(w.width=r),w.height!==i&&(w.height=i)}let se=()=>w.offsetParent===null&&w.clientWidth===0;oe(),new ResizeObserver(oe).observe(w),ae.configure({device:te,format:T,alphaMode:`opaque`}),e.push(`WebGPU: устройство получено, формат — ${T}`);let E=await Dt(te,ae,T);function ce(e,t){return e+`,`+t}function le(e,t){try{let n=window.parent;n&&n!==window&&typeof n.ensureWorldChunk==`function`&&n.ensureWorldChunk(e,t)}catch{}}let ue=new Map;function D(e,t){return[e[0]*t,e[1]*t,e[2]*t]}function de(e,t,n){if(Wt(e,t))return!0;for(let r=0;r<8;r++){let i=r/8*Math.PI*2;if(Wt(e+Math.cos(i)*n,t+Math.sin(i)*n))return!0}return!1}function fe(e,t,n,r){for(let i of x){let a=d.x[i]-e,o=d.y[i]-t,s=(m.get(i)??5)*n+r;if(a*a+o*o<s*s)return!0}return!1}function pe(e,t){return e>.5?t<.62?`spruce`:t<.94?`pine`:`dead`:t<.58?`broadleaf`:t<.8?`birch`:t<.94?`spruce`:`dead`}function me(e,t){let n=[];for(let r=0;r<4;r++)for(let i=0;i<4;i++){let a=e*4+i,o=t*4+r;if(Z(a,o,13122)>=.65)continue;let s=.175+Z(a,o,X+778)*.65,c=.175+Z(a,o,X+779)*.65,l=e*16+i*4+s*4,u=t*16+r*4+c*4,d=l+(Q(l/8.5,u/8.5,X+790)*2-1)*2,f=u+(Q(l/8.5,u/8.5,X+791)*2-1)*2;if(de(d,f,1.5)||fe(d,f,.54,.68))continue;let p=Z(a,o,X+781)*Math.PI*2,m=.85+Z(a,o,X+782)*.3,h=$(d,f);if(h>1.3)continue;let g=h*13,_=.05+.85*Gt(d,f),v=Z(a,o,X+780)<_,y=1+Z(a,o,X+785)*1.3,b=.8+Z(a,o,X+786)*.5;if(v){let e=pe(h,Z(a,o,X+780));e===`broadleaf`&&Z(a,o,13132)<.35&&(e=`autumn`);let t=e===`spruce`||e===`pine`?We:Ge,r=t[Math.floor(Z(a,o,X+784)*t.length)];n.push({x:d,y:g,z:f,scale:[b,y,b],yaw:p,color:D(r,m),kind:e})}else{let e=.1+.30000000000000004*Math.min(1,h/.65);if(Z(a,o,13140)>=e)continue;let t=Je[Math.floor(Z(a,o,X+784)*Je.length)],r=.6+Z(a,o,X+785)*.9,i=.6+Z(a,o,X+786)*.9;n.push({x:d,y:g,z:f,scale:[i,r,i],yaw:p,color:D(t,m),kind:`rock`})}}for(let r=0;r<8;r++)for(let i=0;i<8;i++){let a=e*8+i,o=t*8+r;if(Z(a,o,13232)>=.7)continue;let s=Z(a,o,X+888),c=Z(a,o,X+889),l=e*16+i*2+s*2,u=t*16+r*2+c*2;if(de(l,u,.4)||fe(l,u,.36,.17))continue;let d=$(l,u);if(d>.75)continue;let f=d*13,p=Z(a,o,X+890)*Math.PI*2,m=.8+Z(a,o,X+891)*.4,h=Ke[Math.floor(Z(a,o,X+892)*Ke.length)],g=.8+Z(a,o,X+893)*.6;n.push({x:l,y:f,z:u,scale:[g,g,g],yaw:p,color:D(h,m),kind:`grass`})}let r=16/3;for(let i=0;i<r;i++)for(let a=0;a<r;a++){let o=e*r+a,s=t*r+i;if(Z(o,s,13342)>=.35)continue;let c=Z(o,s,X+998),l=Z(o,s,X+999),u=e*16+a*3+c*3,d=t*16+i*3+l*3;if(de(u,d,.9)||fe(u,d,.44,.34))continue;let f=$(u,d);if(f>.75)continue;let p=f*13,m=Z(o,s,X+1e3)*Math.PI*2,h=.85+Z(o,s,X+1001)*.3,g=qe[Math.floor(Z(o,s,X+1002)*qe.length)],_=.9+Z(o,s,X+1003)*.7;n.push({x:u,y:p,z:d,scale:[_,_,_],yaw:m,color:D(g,h),kind:`bush`})}return n}function O(){let e=[];for(let t of ue.values())e.push(...t);E.setDecor(e),window.__decorCount=e.length,window.__decorList=e}let k=new Set,A=new Set,j=[],he=null,ge=null;function _e(e,t,n=!1){let r=Math.floor(e/16),i=Math.floor(t/16);if(!n&&r===he&&i===ge)return;he=r,ge=i;let a=!1;for(let e=-3;e<=3;e++)for(let t=-3;t<=3;t++){let n=r+t,o=i+e,s=ce(n,o);k.has(s)||A.has(s)||(A.add(s),j.push({cx:n,cz:o,key:s}),a=!0)}let o=!1;for(let e of Array.from(k)){let[t,n]=e.split(`,`).map(Number);Math.max(Math.abs(t-r),Math.abs(n-i))>5&&(E.removeTerrainChunk(e),k.delete(e),ue.delete(e),o=!0)}for(let e of Array.from(A)){let[t,n]=e.split(`,`).map(Number);Math.max(Math.abs(t-r),Math.abs(n-i))>5&&(A.delete(e),a=!0)}a&&(j=j.filter(e=>A.has(e.key)),j.sort((e,t)=>(e.cx-r)**2+(e.cz-i)**2-((t.cx-r)**2+(t.cz-i)**2))),window.__terrainChunkCount=k.size,o&&O()}function ve(e){let t=!1;for(;j.length&&performance.now()<e;){let{cx:e,cz:n,key:r}=j.shift();if(!A.has(r))continue;A.delete(r);let i=e*16,a=n*16,o=$t(i,a,i+16,a+16,1);E.setTerrainChunk(r,o),k.add(r),le(e,n),ue.set(r,me(e,n)),t=!0}t&&(window.__terrainChunkCount=k.size,O())}function ye(e,t){let n=ce(e,t);if(!k.has(n))return;let r=e*16,i=t*16;E.setTerrainChunk(n,$t(r,i,r+16,i+16,1))}function be(e,t,n,r){let i=Math.floor(n/16),a=Math.floor(r/16),o=(i-3)*16,s=(i+3+1)*16,c=(a-3)*16,l=(a+3+1)*16,u=e*64,d=t*64;return u>=o&&u+64<=s&&d>=c&&d+64<=l}let M=new Set,N=new Set,P=[],F=null,I=null;function L(e,t,n=!1){let r=Math.floor(e/64),i=Math.floor(t/64);if(!n&&r===F&&i===I)return;F=r,I=i;let a=!1;for(let n=-4;n<=4;n++)for(let o=-4;o<=4;o++){let s=r+o,c=i+n,l=`far:`+s+`,`+c;M.has(l)||N.has(l)||be(s,c,e,t)||(N.add(l),P.push({cx:s,cz:c,rkey:l}),a=!0)}for(let n of Array.from(M)){let[a,o]=n.slice(4).split(`,`).map(Number),s=Math.max(Math.abs(a-r),Math.abs(o-i))>6,c=be(a,o,e,t);(s||c)&&(E.removeTerrainChunk(n),M.delete(n))}for(let e of Array.from(N)){let[t,n]=e.slice(4).split(`,`).map(Number);Math.max(Math.abs(t-r),Math.abs(n-i))>6&&(N.delete(e),a=!0)}a&&(P=P.filter(e=>N.has(e.rkey)),P.sort((e,t)=>(e.cx-r)**2+(e.cz-i)**2-((t.cx-r)**2+(t.cz-i)**2))),window.__farChunkCount=M.size}function xe(e){for(;P.length&&performance.now()<e;){let{cx:e,cz:t,rkey:n}=P.shift();if(!N.has(n))continue;N.delete(n);let r=e*64,i=t*64,a=$t(r,i,r+64,i+64,4,.35);E.setTerrainChunk(n,a),M.add(n)}window.__farChunkCount=M.size}let Se=pn(te,T,E.getShadowResources()),we=new Map;function R(e){let t=we.get(e);return t||(t=un(e).then(e=>fn(te,e)),we.set(e,t)),t}let Te=new Set(Array.from(x,e=>p.get(e)));await Promise.allSettled(Array.from(Te,e=>R(e)));let Ee=new Map,De=0,z=0;for(let t of x){let n=d.x[t],r=d.y[t],i=Ve(n,$(n,r)*13,r,0,m.get(t)??5),a=p.get(t);try{let e=await R(a);Ee.set(t,Se.createInstance(e,i)),De++}catch(t){z++,e.push(`модель: ошибка на ${a} — ${t instanceof Error?t.message:String(t)}`)}}e.push(`модели: загружено ${De}/${x.length}${z?`, ошибок: `+z:``}`),Dn(e),window.__ecsFound=x.length,window.__foundPositions=()=>x.map(e=>({x:d.x[e],z:d.y[e],scale:m.get(e)??5}));let B=i?i.x:t.x,Oe=i?i.y:t.y,V={yaw:0,pitch:.55,dist:42,target:[B,$(B,Oe)*13+2,Oe]},H=sn(w,V);_e(V.target[0],V.target[2],!0),L(V.target[0],V.target[2],!0);let U=performance.now()+40;ve(U),xe(U),e.push(`рельеф: чанков ${k.size} (16×16) + дальних ${M.size} (64×64, шаг 4), в очереди ещё ${j.length+P.length}`),Dn(e),window.__coverageCheck=(e,t)=>{for(let n of k){let[r,i]=n.split(`,`).map(Number),a=r*16,o=i*16;if(e>=a&&e<a+16&&t>=o&&t<o+16)return`near`}for(let n of M){let[r,i]=n.slice(4).split(`,`).map(Number),a=r*64,o=i*64;if(e>=a&&e<a+64&&t>=o&&t<o+64)return`far`}return null},Object.defineProperty(window,"cam",{value:{get tx(){return V.target[0]},set tx(e){V.target[0]=e,H.stopAuto()},get ty(){return V.target[1]},set ty(e){V.target[1]=e,H.stopAuto()},get tz(){return V.target[2]},set tz(e){V.target[2]=e,H.stopAuto()},get dist(){return V.dist},set dist(e){V.dist=e,H.stopAuto()},get pitch(){return V.pitch},set pitch(e){V.pitch=e,H.stopAuto()}}}),window.H=(e,t)=>$(e,t)*13,window.__camState=()=>({yaw:V.yaw,pitch:V.pitch,dist:V.dist,target:[...V.target]}),window.__isAutoOrbiting=()=>H.isAutoOrbiting();let Ae=document.getElementById(`coordX`),Ne=document.getElementById(`coordY`),Pe=document.getElementById(`coordGo`),Ie=!1;for(let e of[Ae,Ne])e.addEventListener(`input`,()=>{Ie=!0});function Be(){let e=parseFloat(Ae.value),t=parseFloat(Ne.value);!isFinite(e)||!isFinite(t)||(V.target[0]=Math.max(-500,Math.min(500,e)),V.target[2]=Math.max(-500,Math.min(500,t)),V.target[1]=$(V.target[0],V.target[2])*13+2,H.stopAuto(),Ie=!1)}Pe.addEventListener(`click`,Be);for(let e of[Ae,Ne])e.addEventListener(`keydown`,t=>{t.key===`Enter`&&(t.preventDefault(),Be(),e.blur())});let G=new Float32Array(16),Ye=[0,0,0],K=document.getElementById(`selected`),q=[.95,.78,.35],Xe=[.42,.78,.46],Ze=[.82,.24,.26],Qe=null,$e=null,et=null,tt=null;window.startFollowMarch=e=>{H.stopAuto(),tt=e},H.onInteract(()=>{tt=null});function nt(e){et=null,$e=e;let t=(h.get(e)??`?`)+` · `+(g.get(e)??`?`),n=d.x[e],r=d.y[e];Qe={x:n,y:$(n,r)*13+(m.get(e)??5)*.9+2,z:r,color:q},window.__markerActive=!0,window.__selectedLabel=t,K.textContent=t,K.style.display=`block`}function rt(){$e=null,Qe=null,window.__markerActive=!1,window.__selectedLabel=null,K.style.display=`none`}function it(e,t){let n=w.width/Math.max(1,w.height),r=Math.tan(ot/2),i=e/w.width*2-1,a=1-t/w.height*2,o=W(Fe(Ye,V.target)),s=W(Le([0,1,0],o)),c=Le(o,s),l=W([i*n*r*s[0]+a*r*c[0]-o[0],i*n*r*s[1]+a*r*c[1]-o[1],i*n*r*s[2]+a*r*c[2]-o[2]]);return{origin:Ye,dir:l}}function at(e,t,n,r){let i=e[0]-n[0],a=e[1]-n[1],o=e[2]-n[2],s=i*t[0]+a*t[1]+o*t[2],c=i*i+a*a+o*o-r*r,l=s*s-c;if(l<0)return null;let u=Math.sqrt(l),d=-s-u;return d<0&&(d=-s+u),d<0?null:d}let ot=.72;function st(e,t){let n=0;for(let r=2;r<=400;r+=2){let i=e[0]+t[0]*r;if(e[1]+t[1]*r-$(i,e[2]+t[2]*r)*13<=0){let i=n,a=r;for(let n=0;n<12;n++){let n=(i+a)/2,r=e[0]+t[0]*n,o=e[2]+t[2]*n;e[1]+t[1]*n-$(r,o)*13>0?i=n:a=n}return{t:a,x:e[0]+t[0]*a,z:e[2]+t[2]*a}}n=r}return null}function ct(e,t){try{let n=window.parent;n&&n!==window&&typeof n.renderCartoucheFor==`function`&&n.renderCartoucheFor(e,t)}catch{}}function lt(e){try{let t=window.parent;t&&t!==window&&typeof t.renderMarchCartoucheFor==`function`&&t.renderMarchCartoucheFor(e)}catch{}}function ut(e,t){let{origin:n,dir:r}=it(e,t),i=w.height/(2*Math.tan(ot/2)),a=null,o=1/0;for(let e of x){let t=d.x[e],s=d.y[e],c=m.get(e)??5,l=[t,$(t,s)*13+c*.5,s],u=Math.hypot(l[0]-n[0],l[1]-n[1],l[2]-n[2]),f=at(n,r,l,Math.max(c*1.15,46*u/i));f!==null&&f<o&&(o=f,a={kind:`entity`,eid:e,t:f})}for(let e of Y){let t=[e.x,$(e.x,e.y)*13+2.2,e.y],s=Math.hypot(t[0]-n[0],t[1]-n[1],t[2]-n[2]),c=at(n,r,t,Math.max(3,40*s/i));c!==null&&c<o&&(o=c,a={kind:`march`,march:e,t:c})}let s=st(n,r);return s!==null&&s.t<o&&(a={kind:`ground`,x:s.x,z:s.z,t:s.t},o=s.t),a}H.onTap((e,t)=>{let n=w.getBoundingClientRect(),r=ut((e-n.left)*(w.width/n.width),(t-n.top)*(w.height/n.height));if(r?.kind===`entity`){nt(r.eid);let e=v.get(r.eid);e&&ct(e.x,e.y);return}if(r?.kind===`march`){rt(),et=r.march.id,window.__selectedMarchId=r.march.id,lt(r.march.id);return}rt(),et=null,r?.kind===`ground`&&ct(Math.floor(r.x),Math.floor(r.z))});let J=0;async function dt(){let e=bn(V.target[0],V.target[2],384);if(!e)return;let t=new Set,n=[],r=new Set;for(let i of e){t.add(i.key);let e=y.get(i.key);if(e!==void 0){if(h.set(e,i.nm),g.set(e,i.lv),_.set(e,!!i.own),$e===e&&nt(e),p.get(e)!==i.model){p.set(e,i.model),m.set(e,i.scale);let t=d.x[e],r=d.y[e],a=Ve(t,$(t,r)*13,r,0,i.scale);n.push(R(i.model).then(t=>void Ee.set(e,Se.createInstance(t,a))).catch(()=>{}))}continue}let a=b(i),o=$(i.x,i.y)*13,s=Ve(i.x,o,i.y,0,i.scale);n.push(R(i.model).then(e=>void Ee.set(a,Se.createInstance(e,s))).catch(()=>{})),r.add(ce(Math.floor(i.x/16),Math.floor(i.y/16))),ye(Math.floor(i.x/16),Math.floor(i.y/16))}for(let[e,n]of Array.from(y))t.has(e)||(r.add(ce(Math.floor(d.x[n]/16),Math.floor(d.y[n]/16))),Me(l,n),Ee.delete(n),p.delete(n),m.delete(n),h.delete(n),g.delete(n),_.delete(n),v.delete(n),y.delete(e),$e===n&&rt());await Promise.allSettled(n),x=Array.from(Ce(l,[d,f]));let i=!1;for(let e of r){if(!k.has(e))continue;let[t,n]=e.split(`,`).map(Number);ue.set(e,me(t,n)),i=!0}i&&O(),J++,window.__ecsFound=x.length,window.__syncCount=J}s&&setInterval(()=>{se()||dt().catch(e=>console.error(`live sync:`,e))},3e3);let Y=[];function ft(){if(!s)return Y=[],[];let e=wn();return e?(Y=e,window.__marchPositions=e,e.map(e=>({x:e.x,y:$(e.x,e.y)*13+2.2,z:e.y,color:e.own?Xe:Ze}))):(Y=[],[])}let pt=document.getElementById(`labels`),mt=new Map,ht=1024;function gt(){let e=new Set,t=w.clientWidth,n=w.clientHeight;for(let r of x){let i=d.x[r],a=d.y[r],o=i-V.target[0],s=a-V.target[2];if(o*o+s*s>ht)continue;let c=$(i,a)*13+(m.get(r)??5)*.6+1.1,l=He(G,[i,c,a]);if(l.w<=.001)continue;let u=(l.x/l.w*.5+.5)*t,f=(1-(l.y/l.w*.5+.5))*n;if(u<-40||u>t+40||f<-40||f>n+40)continue;e.add(r);let p=mt.get(r);if(!p){let e=document.createElement(`div`);e.className=`wlabel`;let t=document.createElement(`div`);t.className=`nm`;let n=document.createElement(`div`);n.className=`lv`,e.appendChild(t),e.appendChild(n),pt.appendChild(e),p={root:e,nm:t,lv:n},mt.set(r,p)}p.nm.textContent=h.get(r)??`?`,p.nm.classList.toggle(`mine`,!!_.get(r)),p.lv.textContent=g.get(r)??``,p.root.style.transform=`translate(${u.toFixed(1)}px,${f.toFixed(1)}px) translate(-50%,-100%)`}for(let[t,n]of mt)e.has(t)||(n.root.remove(),mt.delete(t))}let _t=new Map;function vt(e,t,n,r){if(!r||!n||r<=n)return t;let i=Math.max(0,Math.min(1,(Date.now()-n)/(r-n)));return e+(t-e)*i}function yt(){let e=new Set,t=w.clientWidth,n=w.clientHeight;for(let r of Y){let i=r.battle;if(!i)continue;let a=r.x-V.target[0],o=r.y-V.target[2];if(a*a+o*o>ht)continue;let s=$(r.x,r.y)*13+2.2+1.6,c=He(G,[r.x,s,r.y]);if(c.w<=.001)continue;let l=(c.x/c.w*.5+.5)*t,u=(1-(c.y/c.w*.5+.5))*n;if(l<-60||l>t+60||u<-60||u>n+60)continue;e.add(r.id);let d=_t.get(r.id);if(!d){let e=document.createElement(`div`);e.className=`blabel`;let t=document.createElement(`div`);t.className=`btitle`;let n=document.createElement(`div`);n.className=`bbar atk`;let i=document.createElement(`i`);n.appendChild(i);let a=document.createElement(`div`);a.className=`bbar def`;let o=document.createElement(`i`);a.appendChild(o),e.appendChild(t),e.appendChild(n),e.appendChild(a),pt.appendChild(e),d={root:e,title:t,atkFill:i,defFill:o},_t.set(r.id,d)}let f=i.retreating,p=!f&&i.revealFromRound===0;d.root.className=`blabel`+(f?` retreat`:p?` deploy`:``),d.title.textContent=f?`Отступление`:p?`Развёртывание`:`Бой — раунд `+i.round;let m=Math.max(0,Math.min(100,vt(i.revealFromAttHp,i.attHpLeft,i.revealStart,i.revealAt)/Math.max(1,i.attStartHp)*100)),h=Math.max(0,Math.min(100,vt(i.revealFromDefHp,i.defHpLeft,i.revealStart,i.revealAt)/Math.max(1,i.defStartHp)*100));d.atkFill.style.width=m.toFixed(1)+`%`,d.defFill.style.width=h.toFixed(1)+`%`,d.root.style.transform=`translate(${l.toFixed(1)}px,${u.toFixed(1)}px) translate(-50%,-100%)`}for(let[t,n]of _t)e.has(t)||(n.root.remove(),_t.delete(t))}function bt(e){if(se()){requestAnimationFrame(bt);return}H.isAutoOrbiting()&&(V.yaw=e*15e-5),H.update(e);let t=ft();if(tt!==null){let e=Y.find(e=>e.id===tt);e?(V.target[0]=e.x,V.target[2]=e.y,V.target[1]=$(e.x,e.y)*13+1):tt=null}Ie||(Ae.value=V.target[0].toFixed(1),Ne.value=V.target[2].toFixed(1)),_e(V.target[0],V.target[2]),L(V.target[0],V.target[2]);let i=performance.now()+6;ve(i),xe(i);let a=[V.target[0]+Math.sin(V.yaw)*Math.cos(V.pitch)*V.dist,V.target[1]+Math.sin(V.pitch)*V.dist,V.target[2]+Math.cos(V.yaw)*Math.cos(V.pitch)*V.dist],o=$(a[0],a[2])*13+2;a[1]<o&&(a[1]=o);let s=w.width/Math.max(1,w.height),c=Re(ze(ot,s,.5,300),Ue(a,V.target,[0,1,0]));G=c,Ye=a,E.setVP(c),E.setFog(a,n,r,e/1e3),E.setSunTarget(V.target[0],V.target[2]);{let t=W(Fe(a,V.target)),n=W(Le([0,1,0],t)),r=Le(t,n);E.setSkyCamera(n,r,t,Math.tan(ot/2),s,e/1e3)}if(Se.setFog(a,n,r),et!==null){let e=Y.find(e=>e.id===et);e?Qe={x:e.x,y:$(e.x,e.y)*13+3.2,z:e.y,color:q}:(et=null,Qe=null)}Qe&&t.push(Qe),E.setMarkers(t),window.__marchCount=t.length-+!!Qe,E.frame({r:n[0],g:n[1],b:n[2],a:1},e=>{for(let t of x){let n=Ee.get(t);n&&Se.draw(e,n,c)}}),gt(),yt(),requestAnimationFrame(bt)}requestAnimationFrame(bt),window.__engineReady=!0}kn().catch(e=>{On([`Ошибка: ${e instanceof Error?e.message:String(e)}`]),console.error(e)});