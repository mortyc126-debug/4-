...
// любого рельефа: наклон вдоль X/Z даёт наклон нормали. Было e=0.5,
// откалибровано под старый HMAX=13 ("мелко для плавного результата, не
// настолько мелко, чтобы шум heightAt на этом масштабе давал
// зернистость"). HMAX с тех пор поднят до 32.5 (×2.5, см. её комментарий
// в terrain.ts) — тот же e=0.5 стал выдавать втрое более крутой наклон на
// том же микрорельефе, а склон/нормаль от этого читались "изломанными,
// потрескавшимися" (репорт автора с устройства), хотя сама геометрия
// (позиции вершин) осталась гладкой — это была именно зернистость
// ЗАТЕНЕНИЯ, не формы. e поднят пропорционально росту HMAX (×2.5→1.25,
// округлено до 1.2) — та же логика калибровки, что и раньше: наклон между
// точками, разнесёнными шире, естественно глаже.
...
// Грубые чанки (дальнее кольцо/задник, step>1) читают heightAt() ТОЛЬКО в
// точных узлах редкой сетки — между узлами GPU просто линейно
// интерполирует уже готовые высоты, heightAt() там вообще не вызывается.
// Настоящая река (после дилатации в пайплайне — всё ещё только ~3 клетки в
// поперечнике) регулярно целиком проваливается МЕЖДУ узлами такой редкой
// сетки: ни один угол квада её не задевает, узел получает высоту сухого
// берега — а та высота НЕРЕДКО ОКАЗЫВАЕТСЯ ВЫШЕ настоящей поверхности
// воды (особенно у гор, где перепад высот вокруг русла велик). При отрисовке
// это не "нет воды в данных" — вода в данных честная — а физическое
// перекрытие: грубый слой лежит НАД настоящей водой и рисуется поверх неё
// по глубине. Автор с устройства заметил ровно это ("может конфликт
// текстур, одни накладываются на другие") — проверено численно: до 36%
// точек настоящих рек перекрыты фоновым слоем (шаг 12), 12% — дальним
// кольцом (шаг 4).
//
// Фикс — не высота В точке узла, а МИНИМУМ heightAt по решётке под-выборок,
// накрывающей всю площадь, за которую отвечает этот узел (шаг под-выборки
// заметно уже ширины реки — узкое русло не может проскочить между ними).
// Соседние узлы сетки берут перекрывающиеся площади (радиус = половина
// шага чанка в каждую сторону) — значит любая точка МЕЖДУ двумя узлами
// гарантированно накрыта минимум-выборкой хотя бы одного из них, и
// провалы всё равно утягивают высоту вниз, а не молча теряются.
const COARSE_KERNEL = 6; // под-выборок на сторону (7×7=49 точек) — с запасом уже реки
function coarseMinHeightAt(x: number, y: number, step: number): number {
  const half = step / 2;
  let best = Infinity;
  for (let j = 0; j <= COARSE_KERNEL; j++) {
    const oy = -half + (j / COARSE_KERNEL) * step;
    for (let i = 0; i <= COARSE_KERNEL; i++) {
      const ox = -half + (i / COARSE_KERNEL) * step;
      const e = heightAt(x + ox, y + oy);
      if (e < best) best = e;
    }
  }
  return best;
...
export function buildTerrainPatch(x0: number, y0: number, x1: number, y1: number, step = 1, sink = 0): MeshData {
  ...
  function vertexAt(x: number, y: number): Vert {
    // smooth (step===1, ближние чанки) — точное heightAt в точке, сетка и
    // так совпадает с разрешением данных, минимум-выборка не нужна.
    // Грубые чанки — coarseMinHeightAt (см. её комментарий выше): иначе
    // узкие реки проваливаются между редкими узлами и грубый слой лежит
    // НАД настоящей водой, закрывая её собой.
    const e = smooth ? heightAt(x, y) : coarseMinHeightAt(x, y, step);
    const water = e < SEA;
