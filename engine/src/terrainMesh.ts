...
export function buildTerrainPatch(x0: number, y0: number, x1: number, y1: number, step = 1, sink = 0): MeshData {
  ...
  function vertexAt(x: number, y: number): Vert {
    ...
    //
    // forest/moisture раньше ТАК ЖЕ обрывались в плоский 0 на грубых чанках
    // — идея была "дальнее кольцо всё равно затянуто туманом, точный тон не
    // виден". Реальный баг с устройства: 0 — это не "нейтральный" тон, это
    // САМЫЙ СУХОЙ полюс шейдера (renderer.ts: moist=0 полностью выбирает
    // desertC, forest=0 не подмешивает forestFloorC) — весь дальний рельеф
    // красился сплошной пустыней, ровным оранжево-песочным пятном вместо
    // настоящего леса/луга. Не проявлялось, пока не снизили FOG_K и не
    // подняли дальность камеры (см. историю двух последних коммитов) —
    // теперь дальнее кольцо видно ясно, и артефакт стал заметен: "остров"
    // зелёного ближнего рельефа посреди плоской оранжевой стены. Экономия
    // была и не такой большой, как казалось — FAR_STEP=4 у дальнего кольца
