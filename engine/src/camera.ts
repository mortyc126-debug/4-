...
// Экспортирован — main.ts считает от него дальнюю плоскость персп.-
// проекции (CAM_FAR), см. её комментарий там: сущности/рельеф грузятся в
// радиусе от ЦЕЛИ камеры, а дальняя плоскость — расстояние от ГЛАЗА;
// расхождение между этими двумя точками растёт вместе с MAX_DIST, и
// дальняя плоскость обязана держать его в уме, иначе на сильном
// отдалении честно "загруженный" объект может оказаться дальше глаза,
// чем эта плоскость, и GPU молча срежет его — при том что остальные
// механизмы видимости (отсечение по экрану) считают его видимым.
//
// Фаза 39 — снова понижено, до 100. Автор с устройства: "убери сильное
// отдаление экрана камеры в Мире, так как при сильном отдалении начинаются
// лаги; чтобы при среднем не было лагов". Это ровно та обратная связь,
// которой ждал абзац выше: прирост со 100 до 140 кадры просадил. 100 —
// не выдумка, а последнее значение, на котором мир был проверен и ехал
// ровно.
// Радиусы дальнего кольца (FAR_LOAD_RADIUS/FAR_UNLOAD_RADIUS в main.ts)
// НЕ трогаем: они уже урезаны отдельной правкой (192 клетки), от
// FAR_UNLOAD_RADIUS считается ENTITY_RADIUS, и понижать его — значит
// вернуть прежнюю жалобу "точки пропадают при отдалении камеры". С
// потолком 100 запас до кольца стал почти двукратным вместо прежних 1.37 —
// то есть на предельном зуме мы теперь заведомо внутри честно
// загруженного слоя.
