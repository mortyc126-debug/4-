...
// 4) стена не бесконечна: реальная геометрия — WALL_BAND (200 клеток) от
//    границы, дальше — резкий обрыв в воду (ниже SEA), а не ещё горы:
//    «не нужно создавать бесконечные горы, это будет нагружать мир» — и
...
//    для плавной стыковки, см. пункт про домешивание ниже).
...
const WALL_BAND = 200;    // толщина самого хребта за границей (см. пункт 4)
...
export function heightRaw(x: number, y: number): number {
  ...
  if (d < WORLD_HALF) return Math.max(0.02, Math.min(1, e));
  ...
  // t=1 на WORLD_HALF+WALL_RISE (уже полный пик). Без верхнего предела —
  // пик стены легитимно выше обычного потолка e=1.0 (см. пункт 2 выше).
  const t = sstep(WORLD_HALF, WORLD_HALF + WALL_RISE, d);
  return Math.max(0.02, e * (1 - t) + edgeWallPeak(x, y) * t);
}
