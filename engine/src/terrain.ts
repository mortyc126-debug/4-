...
// Автор проверил на устройстве предыдущую версию этой кривой (гладкая
// степенная функция t^0.75 на ВЕСЬ диапазон суши разом) и дал развёрнутый
// отзыв — оба замечания оказались прямым следствием того, что кривая была
// ОДНОЙ на весь диапазон:
//   1. "поднял вместе с горами вообще высоты всех биомов" — кривая трогала
//      КАЖДУЮ точку суши, включая низкие холмы и побережье, которые никто
//      не просил поднимать.
//   2. "горы выглядят как сглаженные пики" — у степенной кривой с
//      показателем <1 наклон У САМОГО ВЕРХА (t→1) МЕНЬШЕ исходного (для
//      t^0.75 производная в t=1 равна 0.75, не 1) — кривая сама слегка
//      "срезала" пик в купол, вместо того чтобы просто раздвинуть плечи
//      горы под ним, оставив сам пик как в исходных данных.
// Отдельная (уже хорошая) находка автора: подросшие берега у русла при
// том же уровне моря стали читаться как настоящие речные каньоны —
// понравилось, но только во влажных биомах; в сухих просит вернуть как
// было, БЕЗ обрыва на стыке.
//
// Одно и то же решение для обеих правок разом: вместо гладкой кривой на
// ВЕСЬ диапазон — узкий "холм"-добавка (sin², см. bumpShape ниже — ноль
// И нулевой наклон на ОБОИХ концах пояса, поэтому со стыком нет резкого
// перехода) строго в СРЕДНЕМ поясе [ELEV_BUMP_LOW; ELEV_BUMP_HIGH]:
// низина/побережье (t<LOW) и настоящий пик (t>HIGH) остаются исходными
// данными как есть — пик сохраняет всю свою настоящую (запечённую)
// резкость, низина не поднимается вообще. Амплитуда добавки — не
// константа, а сумма БАЗОВОЙ (везде в поясе — исходная просьба "горы
// шире") и ДОПОЛНИТЕЛЬНОЙ, взвешенной по moistureAt(x,y) (уже готовая,
// гладкая, билинейно интерполированная величина 0..1 — та же карта, что
// уже красит биомы в шейдере земли) — во влажных зонах (у русла/болота)
// добавка заметно сильнее (тот самый каньон, который понравился), в
// сухих — близко к минимуму ("как было"), а переход между ними плавный
// по построению: та же непрерывная карта влажности, обрывов взяться
// неоткуда.
//
// Цена: один лишний билинейный сэмпл (moistureAt) на КАЖДЫЙ вызов
// heightRaw — самая горячая функция движка (вершина/нормаль/декор/
// рейкаст). Вдвое дороже голого heightRaw, но всё ещё вдвое ДЕШЕВЛЕ
// heightAtNatural (5 сэмплов), которую в начале этой же сессии убрали
// именно из соображений производительности — не откат того фикса, просто
// сознательный компромисс ради прямой просьбы автора.
const ELEV_PEAK = 2.34;      // тот же ориентир, что и TERRAIN_SHADER (renderer.ts) для текстурных порогов
const ELEV_BUMP_LOW = 0.12;  // ниже — низина/побережье, не трогаем вообще
const ELEV_BUMP_HIGH = 0.55; // выше — настоящий пик, не трогаем вообще (резкость — из самих данных)
const ELEV_BUMP_BASE = 0.12; // базовая добавка в поясе (исходная просьба "горы шире") — даже в сухих зонах
const ELEV_BUMP_WET = 0.25;  // дополнительная добавка по влажности — эффект "каньона", который понравился
// Автор с устройства прислал скриншот: берег у самой воды обрывается
// почти отвесной ГЕОМЕТРИЧЕСКОЙ стеной, не природным склоном. Причина —
// не bump-добавка выше (та начинается только с t>LOW, а обрыв — прямо у
// уреза воды, t≈0), а голый HMAX×2.5 (см. её комментарий выше): настоящий
// берег, который до сих пор читался нормальным склоном, при возросшем
// множителе высоты стал вчетверо круче на том же горизонтальном
// расстоянии. Смягчаем ТОЛЬКО первый короткий отрезок над водой —
// COAST_BASE подобран не на глаз, а точно: 13/32.5 — отношение старого
// HMAX к новому, то есть у самой кромки воды подъём возвращается ровно к
// той крутизне, что была ДО сегодняшних правок (32.5×0.4=13), а к t=LOW
// плавно (smoothstep — нулевой наклон на обоих концах, без излома на
// стыке с bump-поясом выше) выходит на полную новую высоту.
const ELEV_COAST_HIGH = ELEV_BUMP_LOW; // общая граница с bump-поясом — переход без стыка
const ELEV_COAST_BASE = 13.0 / 32.5;   // старый HMAX / новый HMAX
export function heightRaw(x: number, y: number): number {
  ...
  const e = bilinear(elevData, px, py, ELEV_SCALE / 65535);
  // Вода/русло — не трогаем вообще, ни при какой влажности: сам уровень
  // воды и то, что под ним, должны остаться ровно там, где их запекли.
  if (e <= SEA) return e;
  const t = Math.min(1, (e - SEA) / (ELEV_PEAK - SEA));
  if (t < ELEV_COAST_HIGH) {
    const su = t / ELEV_COAST_HIGH;
    const ease = ELEV_COAST_BASE + (1 - ELEV_COAST_BASE) * su * su * (3 - 2 * su); // smoothstep 0..1
    return SEA + (e - SEA) * ease;
  }
  if (t >= ELEV_BUMP_HIGH) return e; // настоящий пик — исходные данные как есть
  const u = (t - ELEV_BUMP_LOW) / (ELEV_BUMP_HIGH - ELEV_BUMP_LOW);
  const s = Math.sin(Math.PI * u);
  const bumpShape = s * s; // 0 и нулевой наклон на обоих концах пояса, максимум (1) в середине
  const amplitude = ELEV_BUMP_BASE + ELEV_BUMP_WET * moistureAt(x, y);
  return e + bumpShape * amplitude;
}
