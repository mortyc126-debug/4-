...
// Было 13.0 — автор попросил горы "в два-три раза выше" (взял середину,
// ×2.5). HMAX — ЕДИНСТВЕННЫЙ множитель, переводящий нормализованную
// высоту e (0..~2.34, см. bake-скрипт) в мировые единицы Y — на него
// завязано абсолютно всё: сам рельеф (terrainMesh.ts), уровень моря
// (SEA*HMAX, там же), площадки под здания/камера/рейкасты (main.ts,
// camera.ts) и аналитическая нормаль рельефа (normalAt, terrainMesh.ts,
// считает наклон именно по heightAt(...)*HMAX) — единая точка правки, а
// не рассинхрон между "формой" и "высотой в метрах" в разных файлах.
// Заодно это честно делает горы КРУЧЕ (тот же e на том же горизонтальном
// расстоянии даёт больший перепад по Y), не только выше — гористый
// профиль будет читаться заметно резче, не только "растянут по вертикали
// без изменения формы". Не проверено визуально — WebGPU недоступен в
// песочнице; единственное, за чем стоит присмотреть на устройстве —
// не появилось ли режущих теней/полос на самых высоких пиках (SHADOW_FAR
// в renderer.ts=220 подобран с запасом ещё для старого HMAX, но не
// пересчитан отдельно под новый).
export const HMAX = 32.5;
