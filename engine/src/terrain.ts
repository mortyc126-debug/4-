...
// рельеф в горы ближе к ней (визуальная стена, см. edgeMountainBoost) —
...
// Горная стена по границе мира (см. WORLD_HALF выше) — квадратное кольцо
// (Math.max(|x|,|y|), не окружность: сама граница -500..500 квадратная,
// круглый вал оставил бы проходимые клинья по углам). EDGE_RAMP_START —
// где стена начинает подниматься; полоса до WORLD_HALF (70 клеток) —
// переходная, достаточно широкая, чтобы не выглядеть обрывом, но и не
// съедающая игровое пространство (реальные координаты городов не выходят
// за ±200, см. WORLD_HALF). ridge() поверх плавного нарастания — иначе
// стена была бы ровным пологом, а не хребтом: та же функция, что и у
// обычных гор ниже (R.mount), просто на масштабе под ширину полосы.
const EDGE_RAMP_START = 430;
function edgeMountainBoost(x: number, y: number): number {
  const d = Math.max(Math.abs(x), Math.abs(y));
  if (d <= EDGE_RAMP_START) return 0;
  const t = sstep(EDGE_RAMP_START, WORLD_HALF, d);
  const jag = 0.5 + 0.5 * ridge(x / 22, y / 22, SEED + 777);
  return t * (0.6 + 0.4 * jag);
}
...
export function heightRaw(x: number, y: number): number {
  ...
  // Стена по краю мира — аддитивно, не подменой e: клампится в 1.0 ниже
  // (Math.min), так что уже существующие хребты/впадины этой полосы не
  // стираются, а просто почти все "пробивают потолок" — визуально ровно
  // то, что нужно горной стене (пики упираются в максимум, а не в идеально
  // гладкую рампу, см. edgeMountainBoost выше).
  e += edgeMountainBoost(x, y) * 1.6;
