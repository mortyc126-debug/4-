...
// Автор про HMAX выше: "не только выше, а в принципе больше, шире там" —
// HMAX сам по себе честно растягивает УЖЕ СУЩЕСТВУЮЩИЙ профиль по вертикали
// (тот же горизонтальный склон даёт больший перепад по Y — горы выше, но
// не крупнее по силуэту), не годится один. Раздвинуть гору в СТОРОНЫ на
// уровне самих данных нельзя — это настоящий DEM (Copernicus), запечённый
// офлайн-пайплайном (см. шапку файла), которого в этом репозитории нет —
// подвинуть форму массива в стороны здесь физически нечем. То, что можно
// честно сделать без риска рассинхрона с рекой/лесом/зданиями (все они
// читают ЭТИ ЖЕ мировые координаты x,y — сдвигать сами координаты нельзя,
// тот же класс бага, что уже чинили с реками этой сессией) — приподнять
// СЕРЕДИНУ диапазона высот (предгорья/склон) заметнее, чем низ и сам пик:
// склон начинает подниматься раньше и заметнее вокруг вершины, силуэт
// горы визуально "разрастается" в стороны, а не торчит одинокой иглой.
// Уровень моря (ELEV_CURVE_MIN=SEA) и высшая точка региона
// (ELEV_CURVE_MAX — тот же ориентир 2.34, что уже использует
// TERRAIN_SHADER в renderer.ts для нормализации в текстурные пороги,
// держать в синхроне при правке) — оба конца кривой закреплены на месте,
// гнётся только середина, поэтому побережье и высота самого пика не
// съезжают. GAMMA<1 — степенная кривая, поднимающая среднюю часть
// диапазона сильнее краёв (0.75 — умеренно, не превращает низкие холмы в
// горы, но заметно раздувает предгорья). Единственная реальная цена —
// один Math.pow() внутри heightRaw(), а он и так уже самая горячая точка
// движка (вершина меша/нормаль/декор/рейкаст) — не новый разряд стоимости
// (не лишние сэмплы, как было бы у настоящего "размытия вширь"), просто
// одна операция поверх уже существующего вызова.
const ELEV_CURVE_MIN = SEA;
const ELEV_CURVE_MAX = 2.34;
const ELEV_CURVE_GAMMA = 0.75;
export function heightRaw(x: number, y: number): number {
  ...
  // Подводная часть/уровень моря — не трогаем вообще: реки/берег не должны
  // сдвинуться ни на пиксель, кривая применяется только к суше.
  if (e <= ELEV_CURVE_MIN) return e;
  const t = Math.min(1, (e - ELEV_CURVE_MIN) / (ELEV_CURVE_MAX - ELEV_CURVE_MIN));
  return ELEV_CURVE_MIN + Math.pow(t, ELEV_CURVE_GAMMA) * (ELEV_CURVE_MAX - ELEV_CURVE_MIN);
}
