...
// верх.
//
// Автор: «максимальная высота — 2/3 от высоты гор, что ограничивают мир»
// (WALL_PEAK — см. ниже). Раньше потолок был откровенно скромным (асимптота
// 1.35 — меньше половины даже среднего WALL_PEAK=3.5) — то, что MOUNTAIN_
// BOOST ниже поднимает АМПЛИТУДУ вклада гор в саму формулу e (иначе новый,
// более высокий потолок просто никогда бы не доставался: softCapElevation
// сжимает СЫРОЕ e, а сырое e без этого буста в потолок 2.2+ и не забиралось
// бы). Асимптота (KNEE+CEIL_EXTRA≈2.4) — заведомо ниже минимальной высоты
// стены по краю мира (не ниже ~2.98, см. edgeWallPeak/WALL_PEAK), граница
// "это естественная гора" / "это стена" (main.ts: e > NATURAL_ELEVATION_CAP)
// остаётся однозначной, с заметным запасом.
const ELEV_KNEE = 0.85;
const ELEV_CEIL_EXTRA = 1.55;
export const NATURAL_ELEVATION_CAP = 2.35;
// Множитель амплитуды вклада гор (октавы, привязанные к R.mount/prominence,
// хребтовый ridge-член, уклон-поправка) — сырое e должно доставать далеко
// за ELEV_KNEE, чтобы softCapElevation вообще имело что сжимать к новому,
// куда более высокому потолку (см. комментарий выше). Подобрано численно
// (см. историю коммита — проверочный скрипт по всей карте, 350×350 точек):
// даёт практический максимум ≈2.34 (цель — 2.333, ровно 2/3 WALL_PEAK) у
// самых "величественных" цепей (prominence и R.mount оба близки к максимуму
// разом — редкое совпадение, не любая гора: проверено, что среди пиков
// выше 1.8 лишь 0.5% оказываются в пределах 0.05 от абсолютного максимума —
// разброс, не повторяющийся потолок). Настоящие низины (R.mount<0.05,
// ~41% карты) буст вообще не задевает — их среднее не сдвинулось
// (0.435→0.439 в проверке), только "гористая" часть карты стала заметно
// выразительнее, а не весь мир целиком.
const MOUNTAIN_BOOST = 6;
...
// ---- Реки ---------------------------------------------------------------
// Автор: «по озёрам, можешь ещё реки рассчитать?». Честная гидрология
// (сток по градиенту, накопление площади водосбора) тут не подходит: у
// heightAt нет памяти о соседях, это чистая функция ОДНОЙ точки (x,y) — нет
// графа "куда стекает вода отсюда", без него не построить одну связную
// нить от истока до моря. Вместо симуляции — тот же приём, что уже красит
// хребты гор (ridge() выше): ridge(shum) даёт ТОНКИЕ V-образные линии ровно
// там, где обычный noise() пересекает середину диапазона (≈0.5) — на горах
// эти линии читаются как гребни (добавляются К высоте), тут — как русла
// (ВЫЧИТАЮТСЯ из высоты). Тот же математический узор даёт и хребты, и
// овраги — только знак другой.
//
// riverRegionAt — не вся карта реками покрыта (иначе получилась бы сплошная
// паутина трещин, не читаемые отдельные русла) — крупный порог, тот же
// приём "пятен", что и у moistureAt/forestMaskAt: только часть регионов
// вообще "речные".
function riverRegionAt(x: number, y: number): number {
  return sstep(0.6, 0.72, noise(x / 240, y / 240, SEED + 940));
}
// riverVeinAt — сама тонкая нить: высокий порог (0.93) на ridge() оставляет
// только САМЫЙ гребень V-образной линии, не всю её толщину — русло, а не
// широкая мокрая полоса.
function riverVeinAt(x: number, y: number): number {
  return sstep(0.93, 0.985, ridge(x / 42, y / 42, SEED + 950));
}
// Глубина выреза — ТОЛЬКО в низинах/предгорьях (elevBand гасит вклад выше
// scree-линии, рекам не место на голых склонах гор) и только там, где
// регион+нить оба "за" (riverRegionAt*riverVeinAt). e тут — уже почти
// готовая высота (после гор/плато/деталей, см. вызов в heightRaw), не
// сырое x,y — вырез должен реагировать на РЕЛЬЕФ под собой, а не только на
// шум: там, где рельеф и так уже низкий (у самого моря), вырез мельче
// (нечего углублять), там, где повыше — глубже, пока не уйдёт под SEA и не
// станет честной водой (см. heightAt/isWater — читают этот же результат).
function riverCarveAt(x: number, y: number, e: number): number {
  const elevBand = 1 - sstep(0.38, 0.58, e);
  if (elevBand <= 0) return 0;
  const strength = riverRegionAt(x, y) * riverVeinAt(x, y) * elevBand;
  return strength * 0.17;
}
...
export function heightRaw(x: number, y: number): number {
  ...
  // выше.
  const amp = 0.2 + 0.84 * R.mount * prominence * MOUNTAIN_BOOST + 0.35 * R.rough;
  e +=
    (noise(X / 27, Y / 27, SEED) * 0.2 +
      noise(X / 13, Y / 13, SEED + 9) * 0.1 +
      noise(X / 6, Y / 6, SEED + 21) * 0.045) *
    amp;
  e += ridge(X / 17, Y / 17, SEED + 37) * 0.33 * R.mount * prominence * MOUNTAIN_BOOST;
  e += R.mount * prominence * MOUNTAIN_BOOST * 0.1 - (1 - R.mount) * 0.05;
  if (R.plat > 0.02) {
