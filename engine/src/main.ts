...
async function main() {
  ...
  // (f=1-exp(-(d·k)²)): снижен примерно вдвое — теперь f≈0.08 на 150
  // клетках (было заметно раньше), f≈0.4 у самого края дальнего кольца
  // (~384 клетки) — лёгкая атмосферная дымка на самом горизонте, не плотная
  // пелена на пол-экрана. Не проверено визуально (WebGPU-канва не читается
  // в этой песочнице никаким способом) — нужна обратная связь с реального
