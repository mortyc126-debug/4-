...
async function main() {
  ...
  //
  // Второе условие — заслонка от родителя. Автор: "почта чуть подвисает из
  // вкладки мира". Замер показал, что сама почта дешёвая (единицы
  // миллисекунд), а вот движок под ней продолжал рисовать полный кадр
  // шестьдесят раз в секунду: модалка перекрывает экран, но канву не
  // схлопывает (background у неё rgba(...,.72), мир нарочно просвечивает),
  // так что offsetParent остаётся, и isHidden не срабатывал. На телефоне
  // этот кадр и есть весь бюджет — отсюда и рывки при прокрутке письма.
  // Родитель ставит window.__world3dPaused, пока поверх мира открыт
  // полноэкранный экран (см. mpPauseWorld3D в index.html). Кадр просто
  // пропускается: канва сохраняет последнее нарисованное, поэтому мир под
  // полупрозрачной модалкой выглядит ровно как раньше — просто застывшим.
  // Обращение к parent в try: если страница когда-нибудь откроется не в
  // iframe (или из другого источника), чтение бросит, и мы честно считаем,
  // что паузы нет.
  const parentPaused = () => {
    try { return (window.parent as any)?.__world3dPaused === true; }
    catch { return false; }
  };
  const isHidden = () => (canvas.offsetParent === null && canvas.clientWidth === 0) || parentPaused();
  resize();
