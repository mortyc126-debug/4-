...
async function main() {
  ...
  // FAR_CHUNK_SIZE = 384 клетки, с запасом покрывая даже MAX_DIST=100 при
  // пологом наклоне камеры (см. camera.ts и комментарий у FAR_UNLOAD_RADIUS
  // ниже: уже на дефолтном зуме видимая земля уходит на ~78 клеток, а при
  // отдалении — за 150-200).
  const FAR_CHUNK_SIZE = 64;
  const FAR_UNLOAD_RADIUS = 6;
  ...
  // дальнее грубое кольцо тянется до 384 (см. FAR_UNLOAD_RADIUS выше).
  ...
  // друг с другом). FOG_K — квадратично-экспоненциальный коэффициент
  // (f=1-exp(-(d·k)²)): у камеры почти не виден (f≈0.04 на 50 клетках), к
  // краю дальнего кольца рельефа (~250-300 клеток, см. FAR_UNLOAD_RADIUS)
  // уже заметно затянут (f≈0.7-0.8) — подобрано по формуле, не проверено
