...
async function main() {
  ...
  function draw(tMs: number) {
    ...
    const aspect = canvas.width / Math.max(1, canvas.height);
    // Дальняя плоскость персп.-проекции была 300 — с прежними невысокими
    // горами и тяжёлым туманом (FOG_K) хватало: всё, что реально видно,
    // и так тонуло в дымке задолго до 300. FOG_K с тех пор урезан почти до
    // нуля по прямой просьбе автора ("хочу просматривать весь мир, но
    // можно сделать прорисовку как ГТА5") — задник (BACKDROP_STEP) с тех
    // пор честно строится на ВЕСЬ мир (до WORLD_HALF_X/Z, terrain.ts), а
    // не только на дальнее кольцо. Горы к тому же теперь заметно выше и
    // шире (HMAX, ELEV_CURVE_* в terrain.ts) — то есть именно то, что
    // хочется разглядывать издали, раньше всех остальных упиралось в
    // старый far=300: гора/берег на диагональном краю мира при типичном
    // положении камеры (может стоять где угодно в пределах ±1200×±600)
    // рисковали быть банально СРЕЗАНЫ дальней плоскостью, а не мягко
    // растворены туманом. WORLD_HALF_X/Z из terrain.ts — 1200×600, полная
    // диагональ мира ~2683 — far поднят с запасом выше этого потолка, так
    // что задник виден целиком с любой точки под любым углом, обрыв
    // происходит только там, где его и планировали (дымка на самом краю,
    // см. FOG_K), не раньше по вине камеры. depth24plus — near/far здесь
    // растут в разы, но перспективная нелинейность глубины и так держит
    // точность у near (0.5) — где она нужна для близкой геометрии
    // (здания/декор), задник и так грубый (BACKDROP_STEP=12) и не требует
    // точной глубины на таких расстояниях.
    const CAM_FAR = 2800;
