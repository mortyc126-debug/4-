...
async function main() {
  ...
  // ДВА раунда, каждый следующий — минус ПОВЕРХ уже урезанного прошлым
  // раундом (автор прямо уточнил: "это ещё минус я написал"), не заново
  // от исходной плотности (0.05/0.9 дерево, 0.1/0.4 камень, 0.7 трава,
  // 0.35 куст):
  //   раунд 1 — камни/трава/кусты −50%, деревья −30%;
  //   раунд 2 — трава/кусты ЕЩЁ −50% поверх раунда 1, камни ЕЩЁ −60%
  //     поверх раунда 1 (деревья во втором раунде не трогали).
  // Итоговый множитель от исходной плотности: деревья ×0.7 (−30% всего),
  // камни ×0.5×0.4=×0.2 (−80% всего), трава/кусты ×0.5×0.5=×0.25 (−75%
  // всего). Множители применены к тем же порогам вероятности, что решают,
  ...
  const ROCK_CHANCE_MIN = 0.02;   // 0.1 × 0.2 — низина/луг
  const ROCK_CHANCE_MAX = 0.08;   // 0.4 × 0.2 — самые крутые участки
  const GRASS_CELL = 2; // трава — своя, более мелкая подсетка (гуще)
  const GRASS_CHANCE = 0.175; // 0.7 × 0.25
  const BUSH_CELL = 3; // кусты — средний ярус между травой и деревьями, своя подсетка
  const BUSH_CHANCE = 0.0875; // 0.35 × 0.25
