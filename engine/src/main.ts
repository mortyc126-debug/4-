...
async function main() {
  ...
  // чанке рельефа, то есть именно во время движения камеры). Проценты —
  // от ИЗНАЧАЛЬНОЙ плотности (0.05/0.9 дерево, 0.1/0.4 камень, 0.7 трава,
  // 0.35 куст), не накопительно от уже урезанных значений: второй раунд
  // (камни −50%→−60%) заменил множитель камня, а не умножил поверх
  // прежнего. Деревья автор не трогал во втором раунде — остаются на
  // −30% с первого раунда. Множители ниже применены к тем же порогам
  // вероятности, что решают, вырастет ли что-то из уже отобранного
  // DECOR_CHANCE-кандидата (для деревьев/камней) или из своей отдельной
  // подсетки (трава/кусты), без изменения самой густоты подсеток
  // (DECOR_CELL/GRASS_CELL/BUSH_CELL). Точное соответствие проценту
  // приблизительное только для камня: деревья и камни делят один и тот же
  // пул кандидатов (см. isTree ниже) — снижение TREE_CHANCE увеличивает
  // долю кандидатов, которым вообще достаётся бросок на камень, так что у
  // самой границы почти сплошного леса фактическое падение числа камней
  // будет чуть меньше заданного. Трава/кусты — независимые подсетки, там
  // множитель точный.
  const TREE_CHANCE_MIN = 0.035; // 0.05 × 0.7 (−30%) — вне лесного пятна
  const TREE_CHANCE_MAX = 0.63;  // 0.9 × 0.7 (−30%) — в сердце лесного пятна
  const ROCK_CHANCE_MIN = 0.04;  // 0.1 × 0.4 (−60%) — низина/луг
  const ROCK_CHANCE_MAX = 0.16;  // 0.4 × 0.4 (−60%) — самые крутые участки
  const GRASS_CELL = 2; // трава — своя, более мелкая подсетка (гуще)
  const GRASS_CHANCE = 0.35; // 0.7 × 0.5 (−50%)
  const BUSH_CELL = 3; // кусты — средний ярус между травой и деревьями, своя подсетка
  const BUSH_CHANCE = 0.175; // 0.35 × 0.5 (−50%)
