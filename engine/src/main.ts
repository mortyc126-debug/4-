...
async function main() {
  ...
  // прячет. Урезано до 320 (5×64) — тот же порядок, что раньше был у
  // FAR_LOAD_RADIUS. За пределами кольца — не пустота, а мировой задник
  // (main.ts, BACKDROP_STEP, см. его же историю) — он появился уже ПОСЛЕ
  // того, как кольцо разрослось до 448, и с тех пор кольцу незачем тянуть
  // так далеко самому: заметно урезанное количество одновременно
  // загруженных дальних чанков (меньше геометрии на кадр, а с новой
  // объединённой отрисовкой, см. renderer.ts mergedTiers, ещё и меньше CPU
  // на пересборку слоя при движении камеры — та же причина, по которой
  // автор ощущал рассинхрон "грузим больше, чем показываем"). MAX_DIST в
  // camera.ts (140) по-прежнему заметно меньше нового кольца (320) —
  // предельный зум по-прежнему полностью покрыт детальным дальним слоем,
  // не только грубым задником.
  const FAR_CHUNK_SIZE = 64;
  const FAR_UNLOAD_RADIUS = 5;
  ...
  const FAR_STEP = 4;
  const FAR_LOAD_RADIUS = 3; // 3×64=192 клетки от камеры (было 5×64=320 — см. FAR_UNLOAD_RADIUS выше, тот же запас в 2 клетки между load/unload, что и раньше)
