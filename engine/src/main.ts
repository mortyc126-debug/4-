...
async function main() {
  ...
  // чанке рельефа, то есть именно во время движения камеры). Автор задал
  // проценты по видам напрямую: камни −50%, деревья −30%, кусты −50%,
  // трава −50% — множители ниже применены к тем же порогам вероятности,
  // что решают, вырастет ли что-то из уже отобранного DECOR_CHANCE-
  // кандидата (для деревьев/камней) или из своей отдельной подсетки (трава/
  // кусты), без изменения самой густоты подсеток (DECOR_CELL/GRASS_CELL/
  // BUSH_CELL) — теми же формулами, что и раньше, просто с более редким
  // "да" в кости. Точное соответствие проценту приблизительное: деревья и
  // камни делят один и тот же пул кандидатов (см. isTree ниже) — снижение
  // TREE_CHANCE увеличивает долю кандидатов, которым вообще достаётся
  // бросок на камень, так что в редких зонах почти сплошного леса у самой
  // границы леса фактическое падение числа камней будет чуть меньше 50%,
  // а не ровно — для травы/кустов (свои независимые подсетки) множитель
  // точный.
  const TREE_CHANCE_MIN = 0.035; // 0.05 × 0.7 — вне лесного пятна
  const TREE_CHANCE_MAX = 0.63;  // 0.9 × 0.7 — в сердце лесного пятна
  const ROCK_CHANCE_MIN = 0.05;  // 0.1 × 0.5 — низина/луг
  const ROCK_CHANCE_MAX = 0.2;   // 0.4 × 0.5 — самые крутые участки
  const GRASS_CELL = 2; // трава — своя, более мелкая подсетка (гуще)
  const GRASS_CHANCE = 0.35; // 0.7 × 0.5
  const BUSH_CELL = 3; // кусты — средний ярус между травой и деревьями, своя подсетка
  const BUSH_CHANCE = 0.175; // 0.35 × 0.5
